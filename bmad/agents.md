...
- **Parallelism cap** — at most **5–6 agents** run concurrently (the Workflow `parallel()` plus the runtime `cpu-2` cap enforce this); the worktree is the shared isolation boundary; the PR is the single serialization point.
