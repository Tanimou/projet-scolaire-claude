...
- **E11 — Standards interop (OneRoster/LTI) + async imports** · `in-progress` · ~M — move bulk import
  ...
  **RED gate (why NOT auto-merged):** `pnpm install` was never run after adding the `@pilotage/imports-core`
  ...
  `integration.roster_source.created`. **Next run should ship E11-S4** (`epic-slice` `[api][worker][web]` P2,
  **no schema**: idempotent sync apply + conflict resolution + 24h rollback + re-run convergence + the S3
  follow-ups above → **on land `E11` is `shipped`**).
