...
**V3 slice ledger — `V3-E05` · AuthN/AuthZ hardening and permission integrity · layer L0 · `in-progress` (2026-08-07)**

| Slice | State |
|---|---|
| **`S-E05-12`** — the **post-authentication redirect target becomes same-origin-only**, on all four portal login forms. `PortalLoginForm.tsx` read `?callbackUrl=` straight off the query string and handed it to `router.push`, so `/parent/login?callbackUrl=https://evil.example/` showed the **genuine** login page, authenticated the parent **for real** (the session cookie IS set), then landed them off-site — the maximally convincing phishing sequence, on a platform whose users are parents of minors. Now one pure, import-free, **edge-safe** `apps/web/src/lib/safe-callback-url.ts` (100 L, 5 executable, the `lib/portals.ts` pattern verbatim — no ADR owed) validates it **once, at the read site**, so **both** sinks inherit one safe binding (`router.push` on the credentials branch, `signIn(…, { callbackUrl })` on the SSO one) — a per-sink guard is a rule a third sink can forget. **The rule the finding itself recommended is exploitable, and that is a measurement, not an opinion:** `raw.startsWith('/') && raw[1] !== '/' && raw[1] !== '\\'` passes four inputs that still navigate off-origin (`/<TAB>/evil.example`, `/<LF>/…`, `/<CR>/…` → `https://evil.example/`; `/<CR><LF>/evil` → `https://evil/`), because the WHATWG parser strips tab/LF/CR from **anywhere** *before* deciding the origin — so the shipped rule adds a **fifth clause, C0 rejected anywhere**, and **no normalisation at all** (the bytes validated are the bytes navigated). Both the briefed rule and its three holes are pinned by test. `DEFAULT_LANDING` is **deleted**, retiring the fifth surviving copy of the four landing paths onto `PORTAL_LANDING` (`PF-103`'s duplication note). Guard = `apps/api/src/shared/quality/open-redirect-gate.spec.ts` (971 L): the 37-row accept/reject matrix driven through a differential WHATWG-`URL` origin oracle, fails-before/passes-after proven **two ways**, DNC-10 executed with six bypass env vars set, and a whole-tree structural scan (366 files, 55 sink call sites, **0 offenders**). Executed: **214/214** jest, `pnpm typecheck` **13/13 exit 0**, `git diff --check` clean. Closes **`PF-102`**; raises no new finding (`Notification.link` was traced through 14 API writers and found server-composed — `PF-104` deliberately **not** opened); six gate-coverage residuals recorded with owners in `docs/spec/features/v3-e05/PROGRESS.md` | ⚠️ **2026-08-07 — this run, needs human review (NOT auto-merged)** |
| `S-E05-1` … `S-E05-11` | ⬜ **unenumerated** — `PF-07`…`PF-11`, `PF-25`, `PF-26`, `PF-46`, `PF-51`, `PF-52`, `PF-53`, `VAL-07` have rows in `docs/daily-improvement-v3/traceability-matrix.md` but **no story in `sprint-01`**. None is implementable without a `sprint-02` authoring run first |

**`V3-E05` is `in-progress`, and `S-E05-12` is the first slice of it to land** — out of numeric order, because it was
authored fresh to close a finding the `S-E06-5` verify panel raised. It does **not** unblock the other eleven.

**`PF-102` is closed in code, not on the deployment.** Nothing in the slice rebuilds or redeploys; until the operator
rebuilds, the hosted login pages still behave as the finding describes. And one entrance is deliberately **not** covered:
next-auth's own `/api/auth/signin/*?callbackUrl=` is public (`middleware.ts:34`), `auth.ts` declares no `pages:` override,
and its same-origin clamp compares against a **header-derived** `baseUrl` (`trustHost: true`, nginx `server_name _` with
`proxy_set_header Host $host`). Different class (cache-poisoning / absolute-link, not the one-click chain `PF-102` was),
recorded rather than asserted, and it must be verified before the finding is called closed at the deployment level.

---

**Next V3 slice → none is enumerated in any in-flight epic. Two candidates; `PF-102` is no longer one of them.**
