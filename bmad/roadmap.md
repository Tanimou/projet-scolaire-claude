...
- **E11 — Standards interop (OneRoster/LTI) + async imports** · `in-progress` · ~M — move bulk import
  ...
  **RED gate (why NOT auto-merged):** `pnpm install` was never run after adding the `@pilotage/imports-core`
  ...
  yet) and never re-fires on async completion → downstream lists stale until next navigation. **Next slice →
  E11-S2** (`epic-slice` `[schema][api][web][a11y]` P2: reconciliation classification + the "Import & sync
  health" panel + `ReconciliationClass`).
