...
**V3 slice ledger — `V3-E01` · Tenant isolation and identity resolution · layer L0 · `in-progress` (2026-08-14) — 4 slices landed, all four in part; **policy coverage is now COMPLETE (50 of 55 base tables, 45 by column + 5 by FK path)** and no base table is outside all three classes — yet the epic is NOT `shipped` and the running application is **still NOT RLS-isolated**, because what remains is the connection cutover (`S-E01-1`), not more policy work**
...
| **`S-E01-1a`** — **the identity seam stops INVENTING a tenant.** `apps/api/src/shared/auth/user-sync.service.ts:9` carried a tenant slug as a module constant, and `:38-56` **upserted that tenant into existence** whenever an authenticated Keycloak subject matched no `UserProfile`, then created the profile inside it in `status: active`. Since ~50 files do `const me = await this.users.ensureUser(jwt)` and then scope everything on `me.tenantId`, **whatever that function returned WAS the tenancy of the request** — tenancy assigned by a constant, never resolved. Live and reachable: `ADR-004` puts every tenant in ONE realm, and the realm export ships **three enabled, unseeded identities**, each of which silently became an `active` member of a tenant its own login created, carrying its realm-role permissions. The branch is now **deleted, not guarded** — no fail-open leg, no `NODE_ENV`, no `ALLOW_*`/`SKIP_*`, and the file reads no environment variable at all (`DNC-10`, mechanised by a source ratchet with a non-vacuity floor and a `__filename` self-exclusion, plus a positive control so the suite cannot go green on a service that refuses everything). An unprovisioned subject gets `UnprovisionedUserError` — **403 and not 401, argued from measurement**: `api-client.ts:111-113` intercepts 401 and redirects to `login?error=session_expired`, and `AppShellRoot.tsx:86` calls `fetchMe()` on every page of all four portals, so a 401 here is an infinite loop captioned by a falsehood, and no token refresh can make a `UserProfile` appear. The refusal **writes nothing** — no `Tenant`, no `UserProfile`, no audit row — and the audit omission is *forced*, not lazy: `AuditLog.tenantId` is required, so auditing a refusal means inventing the tenant id the code just refused to invent. **Three refusal branches shipped where the story specified one** (`no-match`, `ambiguous-email`, `email-bound-to-another-subject`): the old code did `findFirst({email})` → **unconditional** `update({authProviderId})`, which overwrote an existing binding — i.e. any realm subject presenting a victim's email string could *steal* a bound profile. That is now refused, and so is multi-tenant ambiguity (`email` is unique only PER tenant). Ships **`ADR-043`**. **⚠️ Closes `PF-01` half (a) ONLY** — `register.controller.ts` still upserts the same tenant and forks on `D-02`, recorded as **`PF-185`**; the unscoped email lookup is **`PF-186`**; and **`PF-165` half (b) is untouched** — `permissions.guard.ts:28` still admits a profile-less subject through `@RequiresPermission`, so the refusal only bites where a handler body calls `ensureUser` | ⚠️ **2026-08-14 (run 53) — NOT auto-merged, needs human review (P1 · `[auth][security][tenancy][identity]`).** `jest user-sync.service.spec.ts` **22/22** (new, 470 lines — the tenant origin of the entire API had **no test at all** before it); `jest src/modules/identity src/shared/quality src/shared/auth` **1717 pass**; `pnpm typecheck` **13/13 exit 0** (`@pilotage/api` a genuine cache miss that compiled the changed sources and `prisma/seed-demo.ts`); `git diff --check` exit 0; `production-artefact-check` **green after repair** — it went RED mid-run on two *comments* naming the demo slug (rule `A4` scans raw source, comments included) and both were reworded rather than exempted. **Four residuals a human owns, all measured, none fixed:** (1) the three realm identities are seeded into the tenant that **holds the student data**, with a repo-literal password still in `realm-export.json` and `admin@` carrying `school_admin` — the blast radius of an unrotated demo credential moved from an empty tenant to a full one; (2) adoption is by **bare email string**, unscoped by tenant, with `email_verified` parsed and discarded and `status` never consulted — one matching row is treated as consent; (3) a first binding is **irreversible in-repo** — a realm re-import mints a new `sub`, `ensureUser` then refuses `email-bound-to-another-subject` forever, and neither seed re-binds these three (`seed-keycloak-users.ts`'s `DEMO_USERS` covers only the two `@voltaire.fr` accounts), so recovery is hand-written SQL; (4) the refusal reaches the user as the **generic portal error screen**, not the French sentence — `me.ts:34` catches 401 only and `AppShellRoot` awaits `fetchMe()` in a layout server component, so Next redacts the message to a digest in production. **Recommended next: `S-E01-1b`**, which retires residuals 1–3 with mechanisms that already exist |
| `S-E01-3` | ⬜ not started — `VAL-02`. **Unblocked by `S-E01-2b`**: there is now a policy to defeat, so the fail-before/pass-after criterion is satisfiable for the first time. Note it is *partly pre-empted* — `scripts/rls-isolation-check.js` already runs a two-tenant adversarial proof in the gate; what `S-E01-3` still owes is the same adversarial shape **through the application's own query paths**, not through `psql` |
