...
- **E11 — Standards interop (OneRoster/LTI) + async imports** · `in-progress` · ~M — move bulk import
  ...
  **poll not SSE**, no second datastore/Saga, no auto-delete on SIS removal (soft "à vérifier" conflict). **The
  ...
  schema slice carries an additive `db push` operator pre-req (E7/E8/E9 precedent). **Next slice → E11-S1**
  (`epic-slice` `[schema][worker][async]` P1).
