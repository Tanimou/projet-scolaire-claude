...
**V3 slice ledger — `V3-E01` · Tenant isolation and identity resolution · layer L0 · `in-progress` (2026-08-13) — 3 of 5 slices landed, all three in part; the epic is NOT `shipped` and the running application is NOT RLS-isolated**
...
| `S-E01-1` | ⬜ not started — `PF-01`. **Now the critical path**: it owns both the first real `withTenant` call site and the `DATABASE_URL` → `app_user` cutover that turns `S-E01-2b`'s policies from proven-against-a-role into protecting-the-app. **`S-E01-2c` removed one decision from its plate** — the five FK-derived tables are decided and policied, so the cutover meets them settled; what survives is `outbox_event` (ungranted **on purpose**, `PF-185` — never widen the grant to silence a `permission denied`) and the still-incomplete grant set for `tenant`/`permission`/`role`/`role_permission` |
...
**Next `V3-E01` slice → `S-E01-1` — the identity seam, and the connection cutover that makes the policies protect the
...
`S-E01-1`'s to own because a cutover with no trusted tenant claim is a cutover to *see nothing*: (1) resolve the
tenant explicitly, kill the `demo` fallback, introduce the `pending` state (`PF-01`); (2) **write the first real
