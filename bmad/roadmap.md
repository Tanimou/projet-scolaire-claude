...
**Next V3 slice → `S-E04-7` — the remaining call sites move onto the seam, and a blocking gate keeps them there
(`V3-E04`, `[api][audit]`, size M, `G-AUDIT` **primary**, `G-MIGRATION` **no**, no new ADR — `ADR-035` is amended).**
Its `blockedBy` (`S-E04-6`) was satisfied on 2026-08-09. Five things `S-E04-6` hands it, none of which it may
silently re-decide.

1. **The seam exists but is not exclusive, and that is the whole of `S-E04-7`.**
   `apps/api/src/shared/audit/write-audit.ts` is the one door; **27 direct `auditLog.create` call sites across 15
   files still bypass it**, including `identity/roles.controller.ts`, the exact defect the seam's own docstring
   cites as its motivation (it creates the role, then writes its audit row in a separate, non-transactional
   statement). Migrate them, and land the **pinned-inventory gate** the gate agent specified: an *allow-list* of the
   known-remaining sites, not a ban — a ban is RED on arrival. A new file or a higher count fails pointing at
   `writeAudit`; migrating a site fails too, so the inventory only ever shrinks deliberately.
2. **The vocabulary gate's floor was redefined, not raised on the same definition.** `EXTRACTED.seams` now counts
   *direct writes + shared-seam calls*, so `S-E04-7`'s migration conserves the total by construction and cannot read
   as a regression. Floor is **30**. Do not re-narrow it to the direct form.
3. **`writeAudit`'s `action` / `resourceType` are typed `string`, not the declared unions.** The escalation panel's
   one judgement call: `apps/api` already depends on `@pilotage/contracts`, so typing them
   `AuditActionCode` / `AuditResourceTypeCode` converts `ADR-035` D6 from convention to invariant and matches D1.
   Cheap here, and it removes the "call it with an inline object literal" rule that today has nothing behind it.
4. **`PF-121` / `PF-122` / the remaining `PF-123` sites are still yours.** `S-E04-6` closed `PF-123`'s write half
   **for `assessments.controller.ts` publish only**, because that line was being edited anyway. `PF-150`
   (`portal = ''`), `PF-129`, `PF-132` and `PF-141` remain.
5. **Four postures `S-E04-6` stated out loud rather than fixed** — do not "tidy" any of them without a decision:
   the grantor's own privileges are still not checked against the role granted (`ADR-015` territory), the enrollment
   capacity check is still a TOCTOU read outside the transaction (`PF-154`), the school-create 409 still echoes the
   submitted `schoolCode`, and `PATCH /schools/:id { status: 'closed' }` is a second closure path recorded as
   `school.update` (`PF-155`).

**A human still owns `S-E04-5`'s merge conditions, and now `S-E04-6`'s posture decision too.** `S-E04-5`: `PF-144`
(two `@pilotage/ui` props with zero call sites, so `AC-9`'s rendering half is unreachable), `PF-145` (a malformed
date parameter renders as a service outage), `PF-146` (a malformed export parameter removes the CSV's lower bound —
an unbounded RGPD extract labelled as 90 days), `PF-149` (full-ICU on `node:22-alpine` asserted, never measured,
fails **closed** tenant-wide). `S-E04-6` adds one: **`ADR-035` D2 is now live in production** — a failed audit
insert throws and rolls back the mutation, so role grant/revoke, school create/update/close, all four enrollment
decisions and grade publish now **fail closed** on audit-table trouble. Intentional, documented, and with no kill
switch by design (DNC-10). That is a new failure mode on privileged admin paths and it needs a human signature.

*(Rewritten 2026-08-09, `S-E04-6` land pass — the fourth such rewrite. The pointer named `S-E04-6`, which this run
shipped. Named rather than quietly overwritten, for the same reason as the previous three: the next autonomous run
reads exactly these two places at Step 1.)*

**V3 slice ledger — `V3-E04` · Audit trail and governance surfaces · layer L0 · `in-progress` (2026-08-09) — 6 of 8 slices shipped**
...
| `S-E04-7` … `S-E04-8` | `todo` — per-slice contract in `docs/spec/features/v3-e04/tasks.md`, status in its `PROGRESS.md`. **`S-E04-7` is next** and is now the *exclusivity* slice: `S-E04-6` built the seam but did not make it the only door — **27 direct `auditLog.create` sites across 15 files still bypass it**, `roles.controller.ts` among them. It inherits `PF-121`/`PF-122`/the remaining `PF-123` sites, `PF-150`, `PF-129`, `PF-132`, `PF-141`, and the pinned-inventory gate specified in § `S-E04-6`. `S-E04-8` (the hash chain) stays last **by product ruling** — a chain computed over provenance that is not yet true would be a cryptographically verifiable record of falsehoods |
