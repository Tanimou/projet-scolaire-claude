...
**Next V3 slice → `S-E04-5` — the KPIs share the table's scope, and the `to` filter includes its own day
(`V3-E04`, `[schema][api][web][truth]`, size M, `G-MIGRATION` **YES** — `Tenant.timezone`).** Its `blockedBy`
(`S-E04-4`) is satisfied by this run. Three things `S-E04-4` hands it, none of which it may silently re-decide.

1. **The `kpis` response shape takes its *second* breaking change in two slices, and the epic's own contract already
   describes the end state.** `S-E04-4` shipped `adminLogins: number | null` (« Non instrumenté » rather than a
   fabricated `0`, per its own §2.6). `docs/spec/features/v3-e04/contracts/openapi.yaml` (`AuditKpis`) and
   `data-model.md` `D-23` rule the opposite and assign it here: `adminLogins` is **removed**, `distinctActors`
   replaces it, and each KPI becomes a `{value, scope}` envelope (`AC-10`). **The contract file is currently stale
   against shipped code** — `S-E04-5` owns reconciling it, and must change `page.tsx` in the same diff.
2. **`sensitiveExports` is defined two ways.** `S-E04-4` implements it as an **action** predicate
   (`AUDIT_EXPORT_ACTIONS` ∪ the legacy `'Export'` alias); the contract defines it **structurally**
   (`resourceType = 'export_job'`). The two genuinely disagree on the seeded data — the legacy fixture row carries
   `action: 'Export'` with `resourceType: 'Résultats'`. Pick one *in writing* before the number moves again.
3. **The un-inventoried read side of `portal` is still open, and is now the last half of `PF-32` nobody owns.**
   `analytics.service.ts` builds the portal facet `where: { portal: { not: null } }`, so a `null`-portal row — which
   `PF-123` writes — is reachable by **no offered filter value at all**. A label map could not fix it and `S-E04-4`
   correctly did not try; a scope slice can.

Also inherited, and cheap to fix here while the file is open: `S-E04-4` left the re-armable `adminLogins` branch
**without** the `portal: 'admin'` scope the deleted query carried, behind a card labelled « CONNEXIONS ADMIN ».
It is unreachable today (`AUDIT_LOGIN_ACTIONS` is empty), but the first slice that instruments login makes that card
over-report on an RGPD surface. See the `S-E04-4` row below.

*(Rewritten 2026-08-09, `S-E04-4` land pass. The pointer named `S-E04-4`, which this run shipped, and the ledger row
below still collapsed `S-E04-4 … S-E04-8` into one `todo` line. Named rather than quietly overwritten, for the same
reason the previous two rewrites were: the next autonomous run reads exactly these two places at Step 1.)*

**V3 slice ledger — `V3-E04` · Audit trail and governance surfaces · layer L0 · `in-progress` (2026-08-09) — 4 of 8 slices shipped**
...
| `S-E04-5` … `S-E04-8` | `todo` — per-slice contract in `docs/spec/features/v3-e04/tasks.md`, status in its `PROGRESS.md`. **`S-E04-5` is next** (its `blockedBy` `S-E04-4` is satisfied by this run, and it is the **first `V3-E04` slice where `G-MIGRATION` triggers** — `Tenant.timezone`); `S-E04-6`'s `blockedBy` (`S-E04-1`, `S-E04-3`) is satisfied too, so either may run |
