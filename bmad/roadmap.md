...
**V3 slice ledger — `V3-E01` · Tenant isolation and identity resolution · layer L0 · `in-progress` (2026-08-14) — 3 of 6 slices landed, all three in part; the epic is NOT `shipped`, the running application is NOT RLS-isolated, and a profile-less subject still passes `@RequiresPermission`**
...
| **`S-E01-1b`** — **the demo identities stop being a standing credential, and `status` starts being enforced** (new, proposed by run 53 out of its own residuals) | ⬜ not started — small, no migration, no cutover: remove the `credentials` block from `infra/keycloak/realm-export.json` and extend `seed-keycloak-users.ts`'s `DEMO_USERS` to cover the three realm identities (which both rotates the password to `KEYCLOAK_DEMO_PASSWORD` and gives them the `updateMany({tenantId,email},{authProviderId})` re-bind the `@voltaire.fr` accounts already have — residuals 1 and 3 in one pass, with a mechanism that exists); then gate adoption on `payload.email_verified === true` and `status: 'active'`, both refusing through the same message and code so no enumeration oracle appears (residual 2). **Recommended before `S-E01-1`**: at the cutover these residuals stop being latent |
