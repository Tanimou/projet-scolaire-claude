...
**V3 slice ledger — `V3-E01` · Tenant isolation and identity resolution · layer L0 · `in-progress` (2026-08-14) — 7 slices landed: five in part (`S-E01-2`, `2b`, `2c`, `2d`, `1a`), one that closes `VAL-02`'s DATABASE half (`S-E01-3` — the application half stays open, see the ledger) and one that clears the cutover's **reference surface** (`S-E01-1b`, `ADR-046` — the authorization join `app_user` could not complete, and `role` re-classified from global to tenant-derived); **policy coverage is COMPLETE (53 of 55 base tables, 45 by column + 7 by FK path + 1 auto-discriminant `tenant`)** — it read `50 … 45 + 5` before `S-E01-1b` closed the derivation **transitively**, and the census moved with **no literal edited** — no base table is outside all three classes, and that coverage is **proven at catalog-enumerated breadth** — four verbs × 50 tables × both directions, with a killed mutant — yet the epic is NOT `shipped` and the running application is **still NOT RLS-isolated**. The adversarial suite says so itself, on its GREEN path: `withTenant` has **0 / 722** production call sites and the owner bypass is asserted as a **present** leak. What remains is the connection cutover (`S-E01-1`), not more policy work and not more proof**
...
**Next `V3-E01` slice → `S-E01-1` — the connection cutover.** *(Updated 2026-08-14 by the `S-E01-1b` run, which
**consumed the pointer** — and did so under an id that was already spoken for.)*
...
its amendment **before** the privilege moves. Carry the `S-E01-2b`, `S-E01-2c` **and `S-E01-2d`** merge
