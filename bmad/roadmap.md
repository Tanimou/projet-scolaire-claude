...
**Next `V3-E01` slice → `S-E01-1b` — retire the residuals `S-E01-1a` created, then `S-E01-1` for the cutover.**
*(Updated 2026-08-14, run 53.)* `S-E01-1a` took the identity half of `S-E01-1`: `ensureUser` resolves or refuses and
...
`tenant`/`permission`/`role`/`role_permission` (`ADR-042 §D4`). Carry the `S-E01-2b`, `S-E01-2c` **and `S-E01-2d`** merge
conditions forward: `app_user` must exist **before** the migration on every long-lived cluster, or the GRANT half
silently never runs — a condition that now covers **50** tables, not 49 or 44 — and **`S-E01-2d`'s backfill has never
