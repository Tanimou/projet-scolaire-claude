...
| **`TOOL-13`** *(gate-hardening track, run 45)* — **a suite that stops existing must not read as green.** The ratchet decided on a **set of failures**, and a test that stops executing is not a failure — so it left the set and the verdict was `✓ no drift.` Measured on this worktree 2026-08-12: `schema-drift-gate.spec.ts` reports **5 pending / 0 failed**, including *« the unmodified repository PASSES — the gate is not red on correct code »*, and `scripts/test-ratchet.js` said green about all five. The pure decision layer is extracted to **`scripts/lib/ratchet-core.js`** (new, 291 lines — no I/O, no `process`, no `console`) so the evidence is **fixture-driven against the exact module the gate runs**, never an env var that would let the input be chosen (`DNC-10`, named and refused in the module header). The baseline gains a **second half**: a per-suite count of tests that DID NOT RUN, over `NOT_EXECUTED_STATUSES = {pending, todo, skipped, disabled}` — *measured*, because `describe.skip` surfaces as `'pending'` and a set of only `'skipped'` counts **zero**, which is how this defect would ship again. A **rise** fails the gate exactly like a new failure; a **suite recorded in the baseline but absent from the run** fails too (the other shape of disappearance — the one behind the finding's `2433 → 2219` denominator drift); a **fall** is reported loudly and never failed, because reddening the author who un-skipped a test gets the gate routed around within a week. Ships **`TOOL-16(a)`** in the same file: a load-failed suite's cause now reaches the operator from `suite.message` (**not** `failureMessage`, which does not exist in jest 29.7.0's `--json` output — the finding as written would have shipped a no-op that prints nothing), ANSI-stripped, truncated with a stated marker. **§5 (`TOOL-11`) and §6 (`TOOL-12`) are DROPPED** under the story's own §4 scope order: `scripts/schema-drift-check.js` and `schema-drift-gate.spec.ts` are untouched and both findings stay **OPEN**. **The baseline's `skipped` block is deliberately ABSENT** — populating it needs `--update` from a complete run, which this slice forbids (~350 s and non-deterministic, `TOOL-16(b)`), and a fabricated number would make the gate *look* armed; until an operator arms it the script prints `⚠ … INACTIVE` and qualifies its green. `scripts/ci-gate.sh` **byte-identical**. No production code, no schema, no endpoint, no contract, no UI. Closes **`TOOL-13`** + **`TOOL-16(a)`** | ⚠️ **2026-08-12 — this run, needs human review (NOT auto-merged); `pnpm typecheck` 13/13 exit 0 (48.8 s, `@pilotage/api` executed fresh), `git diff --check` clean — but the 14 new jest cases are **typechecked and UNEXECUTED** by the gate (the 12 core cases were replayed in plain `node`, 12/12 green), and the skip ratchet ships **INACTIVE on both apps** until an operator runs `--update`. See PROGRESS.md § `TOOL-13`** |
