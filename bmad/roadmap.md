...
**Next V3 slice → `S-E04-6` — five privileged families write their audit row in the same transaction
(`V3-E04`, `[api][audit][transaction]`, size M, `G-AUDIT` **primary**, `G-MIGRATION` **no**, `ADR-035`).** Its
`blockedBy` (`S-E04-1`, `S-E04-3`) has been satisfied since 2026-08-08. Four things the earlier slices hand it, none
of which it may silently re-decide.

1. **Re-run the measurement before writing code.** `tasks.md` § `S-E04-6` counts 6 of 28 audit call sites inside a
   `$transaction`, and three of `AC-2`'s five families writing **no row at all** (role grant/revoke,
   `modules/schools/`, `modules/enrollments/`). Those numbers are 2026-08-08 vintage and three slices have touched
   audit paths since.
2. **`writeAudit(tx: Prisma.TransactionClient, input)` — the transaction client is the *first* parameter**, so
   `writeAudit(this.prisma, …)` is a **compile error** rather than a review convention (`plan.md` §5). That is the
   whole design; do not weaken it to a runtime check.
3. **`PF-96`'s posture is *stated* in `ADR-035`; the relation stays untouched.** `AuditLog` has no FK to `Tenant`
   and audit rows outlive their tenant — a reflex cascade would let a tenant deletion erase its own audit history.
4. **Re-check `ADR-035` against `docs/adr/` immediately before creating the file.** `ADR-036` went to `S-E04-1` and
   `ADR-037` to `S-E04-4`, so the number `architecture-impact.md` §4 reserved may already have moved (`PF-110`'s
   precedence rule: `docs/adr/` is the register of record).

**Before either `S-E04-6` or `S-E04-7`, a human owns `S-E04-5`'s merge conditions.** That slice shipped nine new
findings, four of them cheap and blocking: `PF-144` (two new `@pilotage/ui` props with zero call sites, so `AC-9`'s
rendering half is unreachable and « Non instrumenté » still renders at 30 px mono), `PF-145` (a malformed date
parameter now renders as a service outage on the one page whose thesis is that it does not lie), `PF-146` (a
malformed export parameter silently removes the CSV's lower bound — an unbounded RGPD extract labelled as a 90-day
one), and `PF-149` (full-ICU on `node:22-alpine` is asserted, never measured, and fails **closed** tenant-wide).
`S-E04-7` is also unblocked and inherits `PF-150` alongside `PF-121`/`PF-122`/`PF-123`.

*(Rewritten 2026-08-09, `S-E04-5` land pass — the third such rewrite. The pointer named `S-E04-5`, which this run
shipped. Named rather than quietly overwritten, for the same reason as the previous two: the next autonomous run
reads exactly these two places at Step 1.)*

**V3 slice ledger — `V3-E04` · Audit trail and governance surfaces · layer L0 · `in-progress` (2026-08-09) — 5 of 8 slices shipped**
...
| `S-E04-6` … `S-E04-8` | `todo` — per-slice contract in `docs/spec/features/v3-e04/tasks.md`, status in its `PROGRESS.md`. **`S-E04-6` is next** (`blockedBy` `S-E04-1` + `S-E04-3`, both satisfied since 2026-08-08; `G-AUDIT` is its **primary** gate, `ADR-035` is its ADR, `G-MIGRATION` does **not** trigger); `S-E04-7` is unblocked too and now inherits `PF-150` alongside `PF-121`/`PF-122`/`PF-123`, so either may run. `S-E04-8` (the hash chain) stays last **by product ruling** — a chain computed over provenance that is not yet true would be a cryptographically verifiable record of falsehoods |
