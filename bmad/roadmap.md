...
**Next V3 slice → `S-E04-9` — the invite path stops trading an account for an audit row (`PF-163`, **P1**), and
**then** `S-E04-8`.** *(The sprint that wrote this section named `S-E04-8` next and the routine overrode it at land;
this paragraph is the reconciled version, and `docs/daily-improvement-v3/NEXT.md` is the register of record.)*
`S-E04-8`'s `blockedBy` (`S-E04-3`, `S-E04-6`, `S-E04-7`) **is** fully satisfied as of 2026-08-09 — the override is
not about dependencies. It is the epic's own ruling: `plan.md` §2 orders the chain **last** because a chain computed
over provenance that is not yet true is a cryptographically verifiable record of falsehoods, and `S-E04-7` discovered
`PF-163` — a rolled-back audit write can now strand an enabled Keycloak identity with no `UserProfile`, which the next
login self-provisions into `DEMO_TENANT_SLUG` with realm-derived permissions, breaking the `ADR-002` invariant.
Chaining over that is the shape the ruling forbids, and routine triage says a discovered **P1** becomes a story in the
current layer now. `PF-163`'s row in `traceability/OPEN.md` states both acceptable resolutions; the cheap one is
mechanical. ~~**⚠️ Do `TOOL-01` first**~~ — **`TOOL-01` is done.** It was the blocking prerequisite of this pointer
(`audit-findings-index.md` stopped at `PF-133` while the ledger cited 26 later ids, so a baseline row owned by
`PF-163` would have been *correctly refused* by the new gate). **`S-E04-10` appended all 26 on 2026-08-09** — sourced
from **both** ledgers, 18 from `traceability/OPEN.md` and 8 from `traceability/CLOSED-L0.md`, because a register that
resolved only the open half would still fail to resolve the closed one. `PF-163` is now writable as a baseline owner,
so `S-E04-9` may start on its product half directly.
...
**V3 slice ledger — `V3-E04` · Audit trail and governance surfaces · layer L0 · `in-progress` (2026-08-09) — 8 of 10 slices shipped**

*(The denominator moved from 8 to 10, and that is not scope creep being hidden in an arithmetic edit. `tasks.md`
enumerated `S-E04-1`…`S-E04-8`. Two slices were added **after** the kit was written, both from findings the epic
itself raised: `S-E04-9` (`PF-163`, raised by `S-E04-7`) and `S-E04-10` (`PF-155`/`PF-157`/`PF-158`/`PF-159`, raised
by `S-E04-6`'s edge lens). `S-E04-10` shipped before `S-E04-9` because it carried `TOOL-01`, which `S-E04-9` needs.)*
...
| `S-E04-9` | `todo` ◀ **next** — the invite path stops trading an account for an audit row (`PF-163`, **P1**). Its blocking prerequisite `TOOL-01` was discharged by `S-E04-10`, so `PF-163` now resolves against `audit-findings-index.md` and is writable as a baseline owner. Contract: `PF-163`'s row in `traceability/OPEN.md`, which states both acceptable resolutions |
| `S-E04-8` | `todo` — **last in the epic** — per-slice contract in `docs/spec/features/v3-e04/tasks.md`, status in its `PROGRESS.md`. The hash chain stays last **by product ruling**: a chain computed over provenance that is not yet true would be a cryptographically verifiable record of falsehoods, and `S-E04-1`/`-3`/`-6`/`-7` are what made the provenance true first. `G-MIGRATION` **triggers** — the only migration this epic still owes. It inherits `PF-163`, `PF-164`, `PF-121` (now baselined, still open), the remaining `PF-123` write half, `PF-129`, `PF-132`, `PF-136`, `PF-140`, `PF-141`, `PF-150`, `PF-160` |
