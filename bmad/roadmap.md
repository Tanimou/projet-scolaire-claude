...
**V3 slice ledger — `V3-E01` · Tenant isolation and identity resolution · layer L0 · `in-progress` (2026-08-13) — 2 of 5 slices landed, both in part; the epic is NOT `shipped` and the running application is NOT RLS-isolated**
...
| `S-E01-1` | ⬜ not started — `PF-01`. **Now the critical path**: it owns both the first real `withTenant` call site and the `DATABASE_URL` → `app_user` cutover that turns `S-E01-2b`'s policies from proven-against-a-role into protecting-the-app |
