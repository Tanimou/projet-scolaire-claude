...
**Next V3 slice → `S-E04-4` — one canonical audit vocabulary, declared once, in `packages/contracts`
(`V3-E04`, `[contracts][web][api][seed]`, size M, ships `ADR-037`).** Its `blockedBy` (`S-E04-2`) is satisfied, and
`S-E04-3`, which shipped this run, hands it two things it must not silently re-decide. (i) The absence phrase
`PROVENANCE_UNAVAILABLE` now lives in `apps/web/src/app/admin/audit/audit-labels.ts` alongside `humanizeUserAgent` —
an **intermediate** home the vocabulary slice is meant to retire into `packages/contracts`, not a second canonical
one. (ii) `PF-122`'s fifth orphan actor token (`actorRole: 'admin'`, a value that is not a realm role) is still
written today, so `S-E04-4` will read **two incompatible actor vocabularies out of one column** — measure that before
declaring the enum, or the enum is wrong on the live rows. Read `PROGRESS.md` § *The read side of `portal` was never
inventoried* before touching the facet query: a `null`-portal row is reachable by **no offered filter value at all**,
and that is the half of `PF-32` a label map cannot fix.

*(Rewritten 2026-08-08, `S-E04-3` land pass. The pointer was stale by **two** shipped slices: it named `S-E04-2`,
which landed at `3b4055e`, and it described `S-E04-3` as merely "unblocked", which this run shipped. The ledger row
below was stale in the same way — it collapsed `S-E04-2 … S-E04-8` into one `todo` line — and is now split. Named
rather than quietly overwritten, because the next autonomous run reads exactly these two places at Step 1, and a
pointer that survives a completed run is how a routine re-implements shipped work.)*

**V3 slice ledger — `V3-E04` · Audit trail and governance surfaces · layer L0 · `in-progress` (2026-08-08) — 3 of 8 slices shipped**
...
| `S-E04-4` … `S-E04-8` | `todo` — per-slice contract in `docs/spec/features/v3-e04/tasks.md`, status in its `PROGRESS.md`. **`S-E04-4` is next** (its `blockedBy` `S-E04-2` is satisfied); `S-E04-6`'s `blockedBy` (`S-E04-1`, `S-E04-3`) is now satisfied too |
