...
Next slice → S1.**
- [ ] **S1** — **Verify & harden** the already-built email dispatcher end-to-end (targeted
  `dispatchEmails`/processor test + documented Maildev verification + fix only concrete gaps).
  No new queue/template, **no schema**. *(worker)*
