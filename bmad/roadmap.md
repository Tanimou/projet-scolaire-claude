...
used for a real-time deferral, so take the next free number). **Slices S1→S5 in `tasks.md`; next slice → S1**
(`epic-slice`: the 3 snapshot tables + dirty-queue schema + the worker recompute/drain spine + the
snapshot-first read switch on one aggregate endpoint, behind fall-through).
