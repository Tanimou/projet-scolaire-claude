...
- **E11 — Standards interop (OneRoster/LTI) + async imports** · `in-progress` · ~M — move bulk import
  ...
  **RED gate (why NOT auto-merged):** `pnpm install` was never run after adding the `@pilotage/imports-core`
  ...
  branch is dead for them; guardians still default to `created`). **Next slice → E11-S3** (`epic-slice` `[schema][api][integration]` P2: OneRoster source
  connect + pull + map-to-`ImportBatch`, CSV bundle first, on `integrations.write`; `RosterSource` +
  `ImportOrigin`).
