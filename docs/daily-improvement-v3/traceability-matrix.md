...
| VAL-10 running build SHA / schema version | V3-E02 | S-E02-1 | `in-progress` | `migration-state.spec.ts` (`schemaVersion` assertions) — passed locally | `GET /version` returns `buildSha` (`GIT_SHA`/`BUILD_SHA`, 12 chars, `unknown` fallback) + `schemaVersion` (last applied migration) + counts. Exposes **no tenant data and no connection string** — same trust level as the pre-existing public `/`. **Not closed:** R-05 also wants a *deploy gate that compares* the running SHA against the expected one; only the exposure half exists | G-MIGRATION |
...
|---|---|---|---|---|
| L0 | 46 *(43 + PF-63, PF-65, PF-67)* | 4 *(PF-55, PF-58, PF-60, PF-61)* | 41 | 1 *(PF-59)* |
...
|---|---|---|---|---|---|
| 2026-08-02 (run 6) | **S-E02-2 — independent verification, no new PR** | none — **PF-67 advanced, not closed** | — | ✅ `pnpm --filter @pilotage/api build` (exit 0) | The selection rule pointed at `S-E02-2`; this run re-derived the whole diagnosis from `main` **before** discovering PR #172 already existed, which makes it an independent replication rather than a re-read. Confirmed on `main`: the two grading controllers are imported by nothing, `grades.module.spec` fails `2/2`, and the suites run fine with **api 13 red / 435, worker 7 red / 153** — the api figure reconciles exactly with #172's 11 (13 − the 2 tests its fix repairs). Re-executed the ratchet on the branch: **api 424/435, worker 146/153, no drift**; build exit 0; `dist/modules/grades/grades.module.js` contains the `controllers:` array. **Advanced PF-67 without closing it:** statically resolved all five injected providers for the two remounted controllers, which retires the specific worry that #172 could crash the API at boot — `NotificationsService` was the one real candidate (`GradesModule` never imports `NotificationsModule`) and is safe *only* because that module is `@Global()`, a latent trap since de-globalising it would break startup with no test catching it. PF-67 as written asks for a **booted route table**, a stronger and more general guard than a hand-resolved provider list for one module, so it stays `open`. Two corrections filed on the PR: `SchoolStructureModule` is an unnecessary import, and the body's "568/588" should read **570/588**. **No second PR opened, deliberately** — it would push the gate to `FULL` (halting the routine) and conflict with #172 on these very ledger files, while the P0 stayed unmerged. `gh pr merge` **denied again**; no workaround attempted. Verification posted as [a PR comment](https://github.com/Tanimou/projet-scolaire-claude/pull/172#issuecomment-5160775557). **The single blocking action remains one human squash of #172.** |
