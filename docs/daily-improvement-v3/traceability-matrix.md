...
| PF-60 sprint workflow silently overrides the routine's story selection | V3-E02 | — *(tooling defect — story to be written)* | `open` | n/a — observed at runtime | `bmad/workflows/sprint.workflow.js:138` resolves `const epicId = (intake && intake.epic) \|\| ARG_EPIC`, so the intake agent's own choice **wins over the operator override** passed in `args` (the prompt at line 125 only asks it to "honor it unless clearly unsafe"). On the 2026-08-02 run the routine selected `V3-E02 / S-E02-1` and the workflow began implementing `V3-E06 / S-E06-2` instead; caught by inspecting `journal.jsonl` and stopped before anything was written. **This is V3's selection rule becoming decorative — the exact V2 failure mode V3 exists to prevent.** Fix: `ARG_EPIC \|\| intake.epic`, and fail loudly when the intake disagrees with an explicit override | G-DNC |
