...
- **Raises** `PF-409` (nothing links `Enrollment.academicYearId` to `ClassSection.academicYearId`) · `PF-410` (the
  ...
  grade*, that the parent portal renders) · `PF-412` (`classes.controller.ts:306` counted without `tenantId` —
...
| **`PF-412`** | P3 | `classes.controller.ts:306` — `enrollment.count({ where: { classSectionId: id } })` **without `tenantId`** | **CLOSED by this slice** — `DistinctStudentsWhere` requires `tenantId` in the type |
