...
**Status (2026-08-10, after `S-E04-11`)** — **`in-progress`. 10 of 11 slices shipped.**
...
evidence in § `S-E04-9` below. **`S-E04-11` shipped** the regulator-facing half nobody had checked: the `audit_csv`
column list became **append-only** (`PF-140` i — `S-E04-4` had inserted three columns *mid*-header and silently moved
`resource_id`/`ip_address` from 5-6 to 8-9), the collapsed `vocabulary` axis was split into `action_vocabulary` /
`resource_type_vocabulary` **appended at the end** while the summary column stays and is asserted derivable from them
(`PF-140` ii), `csvEscape` now neutralises a formula-leading cell in a file the BOM makes French Excel open as a
*spreadsheet* (`PF-140` iii), and `UnknownTimezoneError` — caught **nowhere** on either audit path — became a named
`503 { code: TENANT_TIMEZONE_UNUSABLE }` on the API and a terminal BullMQ `UnrecoverableError` in the worker
(`PF-149`) — evidence in § `S-E04-11` below. **`S-E04-8` alone remains open**, and every gate it owns is still
...
## Slice backlog (`tasks.md` is the contract; 10 of 11 have shipped)

> **The denominator moved from 8 to 10 to 11, and that is not scope creep hidden in an arithmetic edit.** `tasks.md`
> enumerates `S-E04-1`…`S-E04-8` and is the contract for those eight. **Three slices were added after the kit was
> written, all from findings this epic itself raised** — `S-E04-9` (`PF-163`, raised by `S-E04-7`'s escalation panel),
> `S-E04-10` (`PF-155`/`PF-157`/`PF-158`/`PF-159`, raised by `S-E04-6`'s edge lens, plus `TOOL-01`), and `S-E04-11`
> (`PF-140` + `PF-149` — both left behind by `S-E04-7`, and `PF-149` declined once by `S-E04-6` on a *« the write half
> is vacuous, no writer exists »* measurement that was **true and not sufficient**: `latent` is not `absent`, and the
> *other* half — mapping the error to a named answer — had never been done). None has a `tasks.md` entry;
> `S-E04-10`'s contract is `stories/S-E04-10.md`, `S-E04-11`'s is `stories/S-E04-11.md`, and `S-E04-9`'s is `PF-163`'s
> row in `traceability/OPEN.md`. **`S-E04-10` shipped before `S-E04-9`** because it carried `TOOL-01`, which `S-E04-9`
> needs. **`S-E04-8` is still last, and still the only `todo`.**
...
| **`S-E04-11`** | The regulator-facing export stops changing bytes silently, and stops dying on a value nothing validates (`PF-140` + `PF-149`, both **P1**) | **`shipped`** 2026-08-10 *(AC-1…AC-9 evidenced behaviourally; **`FR-6`/`AC-6` deviated deliberately** — the API answers **503** `ServiceUnavailableException`, not the specified 500, with a written rationale and the tests pinning 503. A human ruling is owed, see § `S-E04-11`. `PF-140` (ii)'s collapsed `vocabulary` column is **kept**, its retirement deferred to a versioned format change)* | `S-E04-4` ✅, `S-E04-5` ✅ | **no** — no schema, no migration, no `packages/contracts` change | — *(`ADR-037` amended: **D6–D7**, and **D4 scoped, not overturned**)* |
...
**11 slices · 1 `todo` · 0 in progress · 10 shipped.** State vocabulary: `todo` → `in-progress` → `shipped`
...
| FR6 | Consumer 3 — the worker CSV **nobody had counted** | `apps/worker/src/modules/exports/generators/audit-csv.generator.ts` — three derived columns added (`action_label`, `resource_type_label`, `vocabulary`); the raw `action` / `resource_type` columns are byte-for-byte unchanged (§2.8: labels are *additional*, a regulator never receives an interpretation in place of the record). **⚠️ Amended 2026-08-10 by `S-E04-11`, and the amendment is the finding this row caused.** Those three columns were inserted **mid-header**, moving `resource_id` / `ip_address` from index 5-6 to 8-9 — a silent shape change in a file a regulator's index-keyed tool reads (`PF-140` i). The current column list is **12 wide**: indices 0..9 are the **frozen prefix**, and `action_vocabulary` / `resource_type_vocabulary` are **appended at 10-11** (`PF-140` ii). **The list is append-only** (`ADR-037` D6): a new column goes at the END; removing or reordering one is a versioned, announced format change, not a commit. « Byte-for-byte unchanged » now reads « unchanged **modulo the `ADR-037` D7 neutraliser** » — a cell whose first character is `= + - @ TAB CR` is force-quoted and prefixed with one apostrophe (`PF-140` iii); every other cell, accented French labels included, is emitted exactly as before |
...
| **`PF-140`** | **CLOSED 2026-08-10 by `S-E04-11`** *(was `open` — regulator-facing artefact; the owner pointer that read `S-E04-7` was stale, `S-E04-7` took the seam half and left this behind)* — **all three sub-findings addressed, with two declared residuals**: the collapsed `vocabulary` column is **kept** (removing it would move `resource_id`/`ip_address` a second time — committing (i) inside the fix for (i)); its retirement is a versioned, announced format change. And `audit_log.user_agent` must **not** join the export until someone re-reads `ADR-037` D7 — the escaper is first-character-only, so a value beginning with whitespace (`" =1+1"`) is not neutralised. Original text follows | Three things about the DPO CSV, none asked for by an AC. (i) `action_label`, `resource_type_label` and `vocabulary` are inserted **mid-header**, not appended, and a UTF-8 BOM is now prepended — any index-based or `created_at`-keyed downstream parser breaks. (ii) The single `vocabulary` column is `weakerVocabulary(action, resourceType)`, collapsing two axes, so a regulator cannot tell **which** value was unclassified — while the epic contract models vocabulary per entry, and the file already carries per-axis label columns. (iii) **Latent, not live:** the BOM promotes the file from "text" to "document French Excel opens as a spreadsheet", and `csvEscape` only quotes on `[",\n]` — it does not neutralise a leading `=`/`+`/`-`/`@`/tab, nor a bare `\r`. No exported column is reachable free text **today**, but `audit_log.user_agent` is a raw client header one column away. Fix: append `action_vocabulary` / `resource_type_vocabulary` at the end, and neutralise leading characters before any slice widens the export |
...
## `S-E04-11` — shipped 2026-08-10 · the regulator-facing export stops changing bytes silently

**Branch** `ci/2026-08-10-v3-e04-s11-audit-export` · **Risk** P1 · **Gates** `G-TRUTH` *(primary)*, `G-DNC`,
`G-TENANT` · **`G-AUDIT` did not trigger** (read path only) · **`G-MIGRATION` did not trigger** — no schema, no
migration, no `packages/contracts` change · **Ships the `ADR-037` S-E04-11 amendment (D6, D7, and D4 scoped)**
· **Contract** `stories/S-E04-11.md`

**Thesis.** *A file a regulator reads must not change shape without someone deciding to change it, and a surface that
fails closed must fail into a named answer, not into an exception nobody catches.* Two P1 findings, one seam (the DPO
audit-export path), **two** tests — a batch, not a bundle.

| Finding | Half it broke | What shipped |
|---|---|---|
| `PF-140` (i) | *does not change shape silently* — `S-E04-4` inserted three columns **mid-header**, moving `resource_id`/`ip_address` from index 5-6 to 8-9 | The list is **append-only** by rule (`ADR-037` D6) and by ratchet: indices 0..9 pinned **by index** and **separately** from the full header, plus a row-width assertion against the header, so *« you moved a column »* and *« you added a column »* are two different red tests |
| `PF-140` (ii) | *the record is not interpreted* — one `vocabulary` column collapsed two independent axes | `action_vocabulary` / `resource_type_vocabulary` **appended at 10-11**, read from the same `ADR-037` resolvers the API predicates and the web adapter read. Column 7 asserted row-by-row as `weakerVocabulary(col10, col11)`, so the summary can never disagree with its parts |
| `PF-140` (iii) | *the file is data, not a program* — a BOM'd CSV is a spreadsheet, and `csvEscape` quoted `" , \n` only | A cell whose first character is `= + - @ TAB CR` is **force-quoted and prefixed with one apostrophe** (OWASP form), **uniformly, never by allowlist**. Additive and reversible; `'` is not itself a trigger, so the step never recurses |
| `PF-149` | *fails into a named answer* — `UnknownTimezoneError` was caught **nowhere** | API: `503 { code: TENANT_TIMEZONE_UNUSABLE, timezone }`, **no `filters` key**. Worker: BullMQ `UnrecoverableError`, graded `failed_terminal` on attempt 1. Both guards span the **whole** window resolution, not the `assertKnownTimezone` line |

**The one thing this run could NOT measure, and it matters.** No browser was driven. `PF-135` recurs for a third
consecutive slice: `/admin/audit`'s behaviour when the API answers **503** is unobserved, and `apps/web`'s `safe()`
helper family treats `[404, 501, 503]` as *« backend not booted yet → degrade to an empty state »* at
`parent/children/claim-actions.ts:38` and `admin/child-claims/page.tsx:16`. That helper is **not** on the audit path
today, but the collision is real and recorded here rather than discovered later.

**Deviation carried for a human, not hidden.** Story `FR-6` / `AC-6` specified `InternalServerErrorException` (**500**)
and pinned `getStatus() === 500`. The code ships `ServiceUnavailableException` (**503**) with a written rationale, and
the two new tests pin 503. Both readings are defensible and genuinely different — 503 is honest about *meaning*
(detected, refused, unusable until an operator edits a row) but is the status proxies, uptime probes and retry logic
read as *transient*, for a condition that is **permanent** until a human edits `Tenant.timezone`. `ADR-037`'s new
amendment records the deviation without ruling on it. **Whichever status wins, one of the two must move.**

**Declared residuals, none of them silent.**

1. The collapsed `vocabulary` column is **kept**. Removing it would move `resource_id`/`ip_address` a second time —
   committing `PF-140` (i) inside the fix for `PF-140` (i). Its retirement is a versioned, announced format change.
2. `audit_log.user_agent` must **not** join the export before someone re-reads `ADR-037` D7: the escaper is
   **first-character-only**, so `" =1+1"` (leading space) is not neutralised. No exported column can begin with
   whitespace today.
3. `isUnknownTimezoneError` is **hand-duplicated verbatim** in `analytics.service.ts` and `audit-csv.generator.ts`.
   It is a predicate about a **contracts-owned** error and belongs beside the class in
   `packages/contracts/src/audit/window.ts`; contracts was out of scope for this slice. **One predicate, two copies —
   the drift class this epic exists to remove.** Follow-up.
...
7. The `audit_csv` export is capped at `take: 50_000` with **no truncation signal** — pre-existing, but now sitting
   inside a column contract this slice froze, so signalling it later is a versioned format change. Do **not** silently
   raise the cap.
8. `audit-vocabulary-gate.spec.ts`'s new `Tenant.timezone` ratchet detects the validator at **file** scope
   (`test(source)`), not at the write payload, and its writer-detection window is a fixed 600-char slice. Zero writers
   today, so it is latent — but every future guarantee rests on that regex. Tighten to the matched payload window.
9. The `name === 'UnknownTimezoneError'` disjunct — the branch that actually runs in the built image, because
   api/worker resolve `contracts/dist` while the specs are mapped to `src` — is exercised by **no test**. Every test
   reaches the `instanceof` branch. If it regresses, the job dies as a plain `Error` and BullMQ retries a
   configuration fault three times, shipping green.
