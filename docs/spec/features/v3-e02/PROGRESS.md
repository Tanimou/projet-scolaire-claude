...
| **TOOL-17(b)** | A tolerated skip must never make a rule **PASS** — a *named* path read out of a tolerant map stops being served as `''`, and the vanish budget stops being flat | ✅ done *(gate-hardening track; **typechecked, NOT executed under jest**; two residuals named below)* | 2026-08-13 (run 48) | **All three `TOOL-17` residuals closed.** `namedReader(label, map)` in `scripts/lib/walk-read.js` **throws** a greppable `DNC-08 (TOOL-17b)` error **carrying the key** instead of yielding `''` — the **48** `MAP.get('<literal>') ?? ''` occurrences measured at `HEAD` become **49** accessor call sites (`audit-provenance-gate` 19, `audit-vocabulary-gate` **21 sites over 20 lines**, `trust-proxy-dnc10-gate` 8, `portal-landing-gate` 1); `git grep` for the old shape over `apps/**/*.spec.ts` now returns only comments and the deliberate fixture literals. `maxVanishedFor(n)` + `VANISHED_FRACTION = 0.02` replaces the flat `MAX_VANISHED_FILES` at **12** comparison sites — ladder `1→1, 10→1, 50→1, 108→3, 210→5, 300→5, 1000→5`, **never 0** for a non-empty corpus, `MAX_VANISHED_FILES` still the **ceiling**, so the small corpus that exposed the defect (`apps/web/tests`: budget 5 → **1**) tightens while every large-corpus assertion keeps its exact meaning. The **sixth victim** (`apps/api/src/shared/audit/write-audit.spec.ts` AC-9) is routed through `mapWalkedFiles` with the identity **and** the scaled cap in the same `it()` and `expect(calls).toBeGreaterThanOrEqual(10)` kept **verbatim**, plus a new AST companion asserting its **five NAMED reads keep their bare `readFileSync`**. `walk-read-gate.spec.ts` **550 → 807 lines, 19 → 26 `it`/`test` blocks**: AC-1 / AC-2 / **AC-3 the green control** / AC-6 boundaries, and AC-8's `findTolerantMapDefaults()` — a **TypeScript-compiler-parsed**, pure `(source) => Violation[]` classifier with a synthetic RED half and a synthetic GREEN control, run over **110** `apps/**` spec files (floor `>= 100`), the corpus read itself routed through the seam. Parsed and never grepped because this corpus spells the forbidden shape inside **string literals**: a text matcher flags its own needle and the only way back to green is to weaken it, which is `R-30`. `scripts/lib/walk-read.js` **241 → 371 lines**; no production code, no schema, no endpoint, no dependency, **no new module** — the seam stays singular, so **no ADR is owed** ⚠️ **`pnpm typecheck` 13/13, exit 0**, `@pilotage/api` a real cache **miss** (a first run was RED on **13** identifiers mangled by a bulk-conversion regex — `expectnamedExecutable(` — repaired, confirming run green; named rather than hidden). `git diff --check` and `git diff --cached --check` exit 0. **⚠️ No jest run.** `namedReader`, `maxVanishedFor` and the R2 classifier were executed directly via `node` against the shipped module (**0 violations over 110 files**, both synthetic halves behaving as asserted), and every literal key plus every `*_REL` constant was verified present on disk so no currently-green named assertion can flip red — but the 26 cases and the 49 converted sites have **never executed under the runner**. **Two residuals, both open:** `R1` of AC-8 (`TOOL-20`), and a **pre-existing RED on `main`** at `audit-provenance-gate.spec.ts:415` that is **not this diff** — see the `TOOL-17(b)` block at the bottom of this file |
...
## `TOOL-17` — a file that vanishes between `walk()` and `readFileSync` must not take an unrelated suite down at LOAD (run 46, 2026-08-13)

`TOOL-13` ended by recording, from its own gate run, the thing it could not fix: two `node scripts/test-ratchet.js api`
runs on **one unchanged tree** produced **two different failure sets**, and every failure in both was a suite that
failed to **LOAD** with `ENOENT` on a probe file a *different* spec had written into the shared checkout and deleted.
This slice closes that.

**The mechanism, restated once so it is not re-derived.** Two specs plant a probe into the real working tree and remove
it in `afterEach` — `audit-write-gate.spec.ts:689` → `apps/api/src/shared/quality/__audit_write_probe.ts`,
`csv-escape-gate.spec.ts:493` → `apps/web/src/lib/__csv_escape_probe.tsx`. Five other specs `walk()` those same roots
and then `readFileSync` every walked path **at module scope**. Under parallel jest the probe is listed by the walk and
gone by the read; the module-level read throws; jest reports `<suite failed to load>` for the whole suite. **Both sides
are correct code** — each writer proves its gate can go red by making it go red on a real file, and walk-then-read is
the right way to build a corpus. The race is the defect.

### The constraint that decides whether this slice was worth landing

The obvious fix is one line, and it is forbidden:

```ts
try { source = readFileSync(file, 'utf8'); } catch { continue; }   // ← DO NOT DO THIS
```

Four of the five victims carry an explicit comment saying they fail at LOAD **on purpose**, because a gate that cannot
read its corpus must go red rather than report "nothing to check, therefore pass". A blanket catch converts five
deliberate loud failures into green ones — **`DNC-08`**, and a third instance of `PF-146` / `PF-105` in this
repository. So the entire engineering content of this slice is the *narrowness* of the tolerance.

### What landed

| Piece | Why it is that and not something cheaper |
|---|---|
| `scripts/lib/walk-read.js` (241 lines) — the **one** `catch` in the slice, in `readOrConfirmVanished` | Four steps, in order: read → **rethrow the original object unwrapped** if `error.code !== 'ENOENT'` → **rethrow** if `exists(path)` is true again → only then push onto `skipped` and return `undefined`. A read that fails `ENOENT` while the file is *present* is not this race and must never be swallowed |
| It lives at `scripts/lib/`, beside `scripts/lib/ratchet-core.js` | **No new architectural decision, so no ADR is owed.** `ratchet-core.js` (#231) is the same artefact class at the same address: a pure CJS helper required **by absolute path** from a spec under `apps/api/src/shared/quality/` (`test-ratchet.spec.ts:307`). Measured this run: **every** walk root in the repo is `apps/<app>/src`, `apps/web/tests` or `packages/<pkg>/src` (`audit-write-check.js:145`, `csv-escape-check.js:135-142`, `production-artefact-check.js:100`, `link-integrity-check.js:186-198`) — `scripts/` is in none of them, so this file is an input to nothing it fixes and never reaches `apps/api/dist` |
| Tolerance applies to **walk-derived paths only** | `SEED_PATH` in `audit-vocabulary-gate.spec.ts` was **split out of** `WRITER_FILES` and keeps its bare `readFileSync`: a missing seed is the *missing-file* seam, not the *vanishing-walked-file* seam. Every `require()` at the top of the five victims stays **unguarded** for the same reason |
| A restated **accounting identity** at every site — `map.size + skipped.length === walked.length` | The identity that existed before (`EXECUTABLE.size === API_FILES.length`) was **added back with the skips**, not deleted. Deleting it is how a tolerance quietly becomes a hole |
| `MAX_VANISHED_FILES = 5`, asserted at every site | The existing floors are asserted on the walk **LIST** (`>= 300`, `>= 200`, `>= 120`) and a skip shrinks the **MAP** — so identity + floor alone would still pass over a corpus that had emptied itself. The cap transports the floor onto the map. It is a **budget, not a measurement**: two writer specs exist, one probe each, observed ceiling **2**. Deliberately **not** `toBe(0)`, which would relocate the flake from load time to assert time and fix nothing |
| `warnSkipped(label, skipped)` names **every** skipped path, not just how many | A count with no identity is the same "cannot tell" DNC-08 guards against, one layer up. The warn is a supplement — the identity and the cap carry the weight, because this repo already recorded its view of a `console.warn` nobody reads at `trust-proxy-dnc10-gate.spec.ts:312` |
| `walk-read-gate.spec.ts` (550 lines, **17 cases**) | Drives a real scratch tree under the OS temp dir — never the checkout. Includes **AC-5: the PRE-SLICE construction throws `ENOENT` naming the vanished path**, so the red-before lives in the same file as the green-after; `EISDIR` off the real filesystem rethrown with the same code *and* message; a non-`ENOENT` failure aborting the **whole map**; the re-check proven to be asked about the *same* path; a wholesale-missing root walking to **nothing** rather than being absorbed as N skips; and a location/uniqueness rule that also asserts the `ratchet-core.js` precedent still exists, so "this is the documented convention" goes red if the precedent moves |

**Nine sites, not five.** The brief named five victim *files*; reading the code found **nine walked-read sites** in
them. Three are in-test rather than at module scope, so their failure mode was a spurious **RED** instead of a spurious
load failure — the same race, and a fix that converted only the module-level five would have left them live.

### The story spec was wrong on one point, and it is recorded rather than quietly worked around

`docs/spec/features/v3-e02/stories/TOOL-17.md` mandated `apps/api/src/shared/quality/__fixtures__/…` and forbade any
location outside `apps/api/src`, on the stated premise that `tsc --noEmit` would fail with *"file is not under
rootDir"* — *"That was checked, not guessed."* **The premise does not hold for this construction.** The rootDir edge
applies to a TS `import`; every consumer reaches this module through a **computed** `require(join(REPO_ROOT, …))`,
which TypeScript never resolves into the program — the same shape the five victims already use for
`scripts/link-integrity-check.js`. `ratchet-core.js` is the standing proof, and typecheck is green from
`scripts/lib/`. The spec's own §4 also conceded that its mandated path would be **compiled into `apps/api/dist/`** by
`tsconfig.build.json` — test infrastructure inside the shipped API artefact. A **§4.1 CORRECTION** was added to the
story doc rather than leaving the contradiction for the next run to rediscover.

### ⚠️ What this slice does **not** have, stated plainly

**No suite was executed.** `pnpm typecheck` is 13/13 with `@pilotage/api` a real cache **miss** that executed, and
`git diff --check` is exit 0 — but no agent runs jest, so the five converted suites and the 17 new cases are
**typechecked and unproven at runtime**. The first full `test:api` run is the evidence that matters here, and the
honest statement of what this slice delivers until then is: *a typechecked hypothesis about a race, with its
red-before case written down*.

**It reduces the flake rate; it does not remove it.** Step 3 is racy in the opposite direction — a probe re-created
before `exists()` runs makes the helper **rethrow**, i.e. the original load crash, rarer but still possible. Only
hermetic writers (**`TOOL-15`, still OPEN**) remove it. `walk()` bodies are untouched, so a *directory* vanishing
mid-walk still throws from `readdirSync`. On Windows a delete-in-flight can surface `EPERM`/`EBUSY`, which are **not**
tolerated on purpose: every measured occurrence was `ENOENT`, and an `EPERM` sighting is a new finding, not a
regression of this one.

### Three residuals the verify panel confirmed, and this slice did not take

1. **A sixth victim exists** — `apps/api/src/shared/audit/write-audit.spec.ts:416` walks `apps/api/src` (the exact root
   the `__audit_write_probe.ts` is planted in) and does a bare `readFileSync` per walked path in its AC-9 test. It is
   the one remaining hand-rolled walk-then-read in the repo. Its failure mode is a nondeterministic RED in a suite
   unrelated to any diff — the defect this slice exists to close, still live one file over.
2. **FR-4 leaks at the named-constant reads.** The tolerance was supposed to apply only to walked paths, with every
   *named* read still failing loudly. But roughly fifteen reads have the shape `EXECUTABLE.get('<literal>') ?? ''`, and
   they are now served out of a tolerant map — so a skipped named file becomes `''`. Both directions are wrong: some
   assertions pass vacuously over `''`, others (`G3_EXCLUSIONS` at `audit-provenance-gate.spec.ts:409`,
   `audit-vocabulary-gate.spec.ts:1169`) go **RED** on a skip, which is the flake relocated rather than removed. The
   fix is one line per suite: assert the named keys are never among the skips.
3. **The cap is one global number applied to corpora of 800 and of 10.** `portal-landing-gate.spec.ts`'s new
   `EXECUTABLE_TESTS` site walks `apps/web/tests` — **10 files** — and its floor is `>= 1`, so identity + cap permit
   **half** the corpus to disappear while green. Neither probe writer touches `apps/web/tests`, so the expected skip
   count there is exactly **0**: the tolerance buys nothing at that site and costs reach.

Also worth naming because nothing else records it: the accounting identity is **tautological by construction** —
`mapWalkedFiles` sends every input down exactly one of two branches, so `entries.length + skipped.length ===
paths.length` cannot fail. Its residual content is "the build function produced no duplicate keys". **The real
detector of a shrinking corpus is the cap**, and `MAX_VANISHED_FILES = 5` is therefore the exact size of the
false-green window these five gates now accept.

`V3-E02` remains **`code-complete`**. `PF-111` and `PF-113` are still its two residuals; `TOOL-15` and `TOOL-16(b)`
are untouched and still open.

### The pointer, as of 2026-08-13 (run 46)

**Next slice → `TOOL-17(b)`** — close this slice's own three residuals, in the order they are listed above. It is the
same shape the `S-E02-17 → S-E02-18 → S-E02-19` chain used, and it is cheap now that the seam exists: the sixth victim
is a five-line conversion through `mapWalkedFiles`, the FR-4 leak is one `expect(NAMED_RELS.filter(r =>
!MAP.has(r))).toEqual([])` per suite, and the cap becomes per-corpus instead of one shared number. Doing it as its own
slice, rather than widening this one, is what kept this PR reviewable — but leaving it undone means the tolerance is
in place while its two known holes are open, which is the wrong resting state for a merge gate.

**The `S-E01-2b` pointer from runs 44 and 45 is still not spent** — `TOOL-10`, `TOOL-13` and now `TOOL-17` are all
gate-hardening picks that jumped the queue, and the dependency map still makes `E01 → E03` a hard edge. Its
precondition is unchanged and still unmet: a working PostgreSQL on `127.0.0.1:5433`.

**One thing changed about the sequencing argument, though, and it favours finishing this track first.** `TOOL-13`
shipped its skip ratchet **disarmed** because arming it requires a *complete, deterministic* run, and `TOOL-16(b)` says
...
## `TOOL-17(b)` — a tolerated skip must never make a rule PASS (run 48, 2026-08-13)

> **The pointer above, followed exactly.** Run 46 recorded three residuals against `TOOL-17` and run 47 restated them
> in the same order. All three are closed here. Nothing was re-scoped, and one claim in the brief was measured false
> and is recorded as such rather than quietly worked around.

### The one sentence

`TOOL-17` made a walked read tolerate a file that vanished between `walk()` and `readFileSync`. That tolerance is
correct and stays. What it opened was `DNC-08` one layer up, at the **reading** sites: **a file that was skipped must
...
| Piece | Why it is that and not something cheaper |
|---|---|
| `namedReader(label, map)` in `scripts/lib/walk-read.js`, and the **48** `MAP.get('<literal>') ?? ''` occurrences measured at `HEAD` converted into **49 accessor call sites** — `audit-provenance-gate` 19, `audit-vocabulary-gate` **21 sites over 20 lines** (one line carried two reads), `trust-proxy-dnc10-gate` 8, `portal-landing-gate` 1. *(The counts were re-measured at land time: the first draft of this row totalled the parenthetical to «48», which does not add up — it is 48 **occurrences before**, 49 **call sites after**.)* | The leak was `MAP.get('<literal>') ?? ''`: a **named** path served out of a **tolerant** map. If the file was skipped the site yielded `''`, and `''` contains nothing, matches no `toMatch`, and satisfies **every** `.not.` downstream — for ever. The repair is at the accessor, not at 48 assertions: one accessor per corpus map, built beside the map, that **throws** with a greppable `DNC-08 (TOOL-17b)` message **carrying the key**. No assertion changed meaning; the accessor returns exactly what the map holds |
| `maxVanishedFor(n)` + `VANISHED_FRACTION = 0.02`, adopted at **12** comparison sites | `MAX_VANISHED_FILES = 5` was a **flat** budget applied to a **10-file** corpus (`apps/web/tests`, `portal-landing-gate.spec.ts:217`) whose only list floor is `>= 1`: half the corpus could vanish while the gate stayed green. The cap now scales with the walked list and `MAX_VANISHED_FILES` stays the **ceiling**, so every large-corpus assertion keeps its exact meaning. Measured ladder: `1→1, 10→1, 50→1, 108→3, 210→5, 300→5, 1000→5`. **Never 0 for a non-empty corpus** — a zero budget is `expect(skipped).toBe(0)`, i.e. the flake relocated from load time to assert time, which is the defect the module exists to remove |
| The sixth victim: `apps/api/src/shared/audit/write-audit.spec.ts` AC-9 | The last hand-rolled walk-then-read in the repo. Converted to `mapWalkedFiles`, with the accounting identity and the scaled cap asserted **in the same `it()`** and the vacuity guard `expect(calls).toBeGreaterThanOrEqual(10)` kept **verbatim** — the conversion must not change what the test concludes, only stop it being flaky. Its **five NAMED reads** (`SELF_PATH` ×3, the contracts vocabulary, this spec file) deliberately keep their bare `readFileSync`, and a new companion case **asserts over the AST that they do**, so a future over-eager conversion goes RED |
| `walk-read-gate.spec.ts` grows **550 → 807 lines** and **19 → 26** `it`/`test` blocks *(the story's "17 → 23" was inherited from run 46's row and not re-measured; the file's own count at `HEAD` was 19)* | AC-1 (throws, naming `DNC-08`, the site and the exact key), AC-2 (a present key served byte-for-byte, `toBe`), **AC-3 the green control** (over the *same* scratch tree with no skips, every named key is served — the case that proves AC-1 is a defect detector, not a change of behaviour), AC-6 (the cap's **boundaries**, never the formula: the `0.02` is deliberately not asserted), and AC-8's two halves |
| **AC-8 — R2 as a ratchet, PARSED with the TypeScript compiler, never grepped** | `findTolerantMapDefaults(source, fileName?)` is a **pure** `(string) => Violation[]`, so its RED half and its GREEN control are driven over **synthetic** sources in-run rather than over the repo. It flags a `??`/`||` binary expression whose right side is an empty string literal and whose left is a `.get(…)` call; the converted shape, a non-empty default, a non-`.get` call, and **the same spelling inside a string literal** are all correctly not flagged. It then runs over **110** `*.spec.ts` files under `apps/**` (floor asserted at `>= 100`, so "no violations" can never mean "nothing was read"), and that corpus read **itself** goes through the seam with the identity and the scaled cap |

**Why parsed and not grepped, stated once so it is not re-litigated:** this very file, and the specs it judges, spell
`MAP.get(x) ?? ''` inside **string literals and comments** — as fixtures and as design notes. A text matcher flags its
own needle, and the only way to make a text matcher green again is to weaken it. That is `R-30` exactly.

### The measured correction, and the honest miss

- **The brief was wrong about one of the 48 sites, and it is recorded rather than papered over.** It asserted every key
  is a hard-coded literal. `portal-landing-gate.spec.ts:520` keys the map from `[...EXECUTABLE_SRC.entries()]`, so it is
  **always present** and can never take the throw branch. That is not an exception to carve out — it is the proof that
  the throw is reachable **only** via the skip path. It is converted like the other 47 and the reason is written in the
  code, so no reviewer re-derives it.
- **`R1` of AC-8 was NOT implemented, and the escape hatch was taken deliberately** — *"no spec reads a walk-derived
  path with a bare `readFileSync`"*. Deciding it needs local call-graph reachability (`readdirSync` transitively through
  locally-declared functions) plus dataflow from that binding into a `readFileSync` argument: a second classifier of
  comparable size to this whole slice. A grep is **not** an acceptable substitute for it, so nothing was shipped in its
  place. Recorded as a **named residual** in `walk-read.js`'s own `RESIDUALS` section with the reason and the cost.
  Until it exists, a **new** bare walked read is caught by review, not by a gate.
- **Evidence status, stated the way run 46's row states it:** `pnpm typecheck` green; **no jest run in this run**. The
  26 `walk-read-gate` cases and the converted 49 sites are typechecked, not executed. The `namedReader` and
  `maxVanishedFor` boundaries **were** executed directly against the shipped module, and the AC-8 classifier **was**
  executed against all 110 spec files (0 violations, both synthetic halves behaving as asserted) — but through `node`,
  not through jest.

### What the escalation panel found, folded in rather than left on the PR

Three items came back from the review panel. All three are recorded here, because a finding that lives only in a PR
comment is a finding the next run does not read.

1. **`main` is RED at `HEAD`, and it is not this diff.** A reviewer ran the five converted suites twice on one tree and
   got two **agreeing** results (327/328 and 361/362, 4 then 5 suites PASS, the **same** single failure both times;
   `walk-read-gate.spec.ts` alone **34/34**, `it.each` expanded). The failure is
   `audit-provenance-gate.spec.ts:415` — G-3/AC-1, *"the only role-precedence ordering in the whole app is
   `provenance.ts`"* — with two offenders, `apps/api/src/shared/auth/role-ladder.ts` and its spec. Cause:
   `role-ladder.ts:57`, `REALM_ROLE_LADDER = ['student','parent','teacher','school_admin','super_admin'] as const`,
   shipped by **`63f8650` (PR #238, `S-E05-2b`)**, whose evidence line covers the identity + auth suites and **not**
   `audit-provenance-gate`. Attribution is conclusive: `role-ladder.*` is untouched by this diff and the diff contains
   no line touching the G-3 `offenders` computation. **Repair belongs to its own finding**, and it is a third **named**
   entry in `G3_EXCLUSIONS` (`audit-provenance-gate.spec.ts:102`) with the reason written in — never a glob — paired
   with the existing *"the exclusions really DO trip the matcher"* case at `:418` so a new exclusion cannot silently
   protect nothing.
2. **AC-8's R2 classifier is broader than the sentence it enforces**, and the sanctioned narrowing is written down
   here so nobody reaches for the forbidden one. `findTolerantMapDefaults` matches *any* `X.get(k) ?? ''`, with no
   notion of "a corpus map built by the tolerant seam". `searchParams.get('q') ?? ''` and `headers.get(…) ?? ''` are
   live idioms in this repo (`apps/web/src/lib/client-provenance.ts:243`,
   `apps/api/src/shared/keycloak/keycloak-admin.service.ts:154`); they are out of corpus **today** only because the
   corpus is `*.spec.ts(x)`, and the Playwright specs under `apps/web/tests` are in it. **The narrowing, when a false
   RED arrives:** restrict the left-hand receiver to identifiers whose declaration is initialised from
   `mapWalkedFiles` / `new Map(<…>_ENTRIES)` — all four are single-token module-scope `const`s, so it is one collect
   pass. **Deleting or loosening the classifier is `R-30` and is not an option.** Three next-spellings also evade it
   today and two of them are vacuous-PASS rather than RED: `const EMPTY = ''; MAP.get(k) ?? EMPTY` (identifier, not a
   literal), `MAP.has(k) ? MAP.get(k)! : ''` (ternary), and `String(MAP.get(k))` — the worst, because it yields the
   **non-empty** `"undefined"` and so silently satisfies `not.toContain('BYPASS_')`. R2 is complete against exactly
   one spelling; read it that way.
3. **The 49 accessor placements are correct and unguarded.** Every one is deferred inside an `it`/`test`/`before*`/
   `after*` callback — verified by AST at land time — so a skipped named file costs one `it()`. Nothing asserts that
   property. A future site written the natural way beside its map at `describe` scope (`const source =
   namedExecutable('apps/api/src/main.ts');`) throws at **collection** and takes the whole suite down at LOAD, which
   is the failure `#233` closed. The ratchet for it is a third `findTolerantMapDefaults`-shaped classifier in the same
   file, same corpus, same pure signature, ~40 lines; it is folded into **`TOOL-20`** rather than bolted on here.

**Watch item, not a blocker.** `maxVanishedFor` tightens `apps/web/tests` (11 files measured) from a budget of 5 to
**1**. That is the intended direction — a corpus that could lose half itself and stay green is the defect — but
`TOOL-18`'s check-scripts race is closed while the *class* is not, so if a probe ever lands under `apps/web/tests`,
`portal-landing-gate.spec.ts:232` is the first false-RED site. Written here so a future red is diagnosed in seconds.

### The pointer, as of 2026-08-13 (run 48)

**Next slice → arm the skip ratchet.** With `TOOL-15` (probe writers hermetic), `TOOL-17` (the tolerant seam) and
`TOOL-17(b)` (the tolerance can no longer buy a pass) all landed, the two named sources of `apps/api` suite
non-determinism are closed. The remaining step is the one `TOOL-13` shipped **disarmed** on purpose: once `ci-gate.sh`
runs **agree twice** on one tree, `node scripts/test-ratchet.js api --update` (and `worker`) from a **complete** run is
an **operator** step that finally closes `TOOL-16(b)`.

**Then `TOOL-20`** — `R1` of AC-8 (*"no spec reads a walk-derived path with a bare `readFileSync`"*) at the size stated
above, batched with the two cheap ratchets the panel identified: the **accessor-placement** rule (item 3) and the
**receiver narrowing** of R2 (item 2). It is a named `open` row in `OPEN.md` so slice selection keeps seeing it after
the `TOOL-17` row is archived.

**And, separately from this epic's queue: the `main` RED at `audit-provenance-gate.spec.ts:415`** (item 1 above). It is
one named `G3_EXCLUSIONS` entry plus its companion non-vacuity case; it blocks any suite-level evidence for the next
gate-machinery slice, so it should be picked before `TOOL-20`.
