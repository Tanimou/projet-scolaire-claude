...
| **S-E01-4** | Student Keycloak client split (**+ `VAL-04` live review**) | ⬜ **SPLIT 2026-08-15 — `D-02` is DECIDED, not blocked.** The operator override of run 60 took the decision in the direction `ADR-021` had already recorded as its named alternative; the artefact-and-code half is taken by **`S-E01-4a`** below. What remains under this id is **`S-E01-4b`**: the executable gate (`AC-6`/`AC-7`) and `VAL-04` against a **running** Keycloak | 60 | `PF-18` closed by `S-E01-4a` (artefact + code); **`VAL-04` stays open** — no live server was reachable, `TOOL-19` |
