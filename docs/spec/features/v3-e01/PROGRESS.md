...
| **S-E01-1k** | The boot-probe privilege closure stops being **HAND-WRITTEN** and becomes **DERIVED** — and the derivation names, on its first pass, the entry three human re-reads missed | 🟢 **shipped — 2026-08-22, ⚠️ NOT auto-merged (P1 · `[tenancy][security][authz][rls][boot-probe][ci-gate][tooling]`)** — [`stories/S-E01-1k.md`](./stories/S-E01-1k.md) | 69 | **Closes `PF-246`**, advances **`PF-219`**, ships **`ADR-059`**, records **`PF-249`**–**`PF-253`** and **`TOOL-40`**. **Not a module conversion** — `70 + 120 / 818` is unmoved by construction, and no handler changed. `scripts/tenant-adversarial-check.js` gains `derivePrivilegeClosure` + `privilegeClosureDrift`: it walks the call sites attributed **exactly `scoped`** (never a `tx.` grep, which would confuse the tenant scope with the OWNER's `$transaction`), descends the relation depth that `where` / `select` / `include` / `orderBy` / `_count.select` traverse, resolves each relation against `schema.prisma`, and compares the result to `APP_ROLE_REQUIRED_PRIVILEGES` for **set equality in both directions** — the `enumerationDrift` / `ADR-051 §D2` shape, a list of NAMED findings rather than a boolean. Three new pure modules carry the parts: `scripts/lib/js-source-scan.js` (one shared comment/string/template/regex-aware delimiter lexer — `ADR-059 §D6`, the correct read of `TOOL-39`), `scripts/lib/app-role-closure.js` (parses the declared list as **text**, because agents never build so `dist/` is stale by construction — `§D1`), `scripts/lib/prisma-schema-graph.js` (model → table **derived** and *compared* against the live catalog, never a third hand-written catalog — `ADR-042 §D3`). **What the mechanisation found on its first run: `guardian.SELECT`.** The parent ABAC guard of `lessons/list` filters not on a column but on a **relation** (`guardianship.findFirst` whose `where.guardian` is itself an object — `lessons.controller.ts:366`); under RLS a relation filter is a READ of the target table, and `guardian` is **not** a root delegate anywhere in the tree, which is exactly why five slices and three human re-reads never saw it. Closure **37 → 38**, and the literal in `remediation-scope-ownership.spec.ts` is bumped as a **frozen non-vacuity probe**, not as a lower bound — an entry too MANY returns `refused_unusable`, i.e. a 503 on all four portals. **No boot hazard from entry 38, verified on the migration rather than on the dev stack:** `20260813120000_tenant_rls_policies/migration.sql:385` derives its `GRANT SELECT, INSERT, UPDATE, DELETE` loop from the tables carrying a `tenant_id` column, and `Guardian` carries `tenantId @map("tenant_id")` — the grant exists on any migrated database, and any deployment where entry 38 would 503 is already 503-ing on `guardianship.SELECT`. **`ci-gate.sh` trigger widened** to `apps/api/src/modules/` and `scripts/lib/` (`§D3`) — the stage that polices module conversions never ran on one; the cost is a 300 s DB-bearing stage on a much larger share of backend diffs, named in the ADR rather than slipped in as a regex edit. **Executed:** `node scripts/tenant-adversarial-check.js` exit **0**, `38 declared === 38 derived`, both directions, **0 exceptions**, 70 scoped sites all argument-walked, 228 files, 54 models all `@@map`-ed; `AC-9` byte-identical at `70 / 120 / 818`; positional attribution explicitly **refuses** `booking.INSERT`, `booking.UPDATE`, `audit_log.INSERT`; mutation test driven through the exported pure functions over the real corpus — removing `guardian.SELECT` yields `undeclared-pair`, adding a spurious `school.DELETE` yields `dead-entry-advisory`, **both directions bite**; `pnpm typecheck` **13/13**; `git diff --check` exit 0. **Named limits, all recorded rather than glossed:** the **third** list is still unmechanised — nothing asserts `declared ⊆ granted-matrix`, so the next conversion that derives a pair the matrix deliberately withholds (`announcement_receipt.DELETE`, `audit_log.UPDATE`) would follow the gate's own instruction into a boot-green four-portal 503 (**the must-do before the next module lands**); `parseDerivedChildParents(...).problems` is computed and **discarded** at `:3784`, so the `PF-252` parent-SELECT rule can silently switch off if that one hard-coded migration directory is renamed; `VERB_PRIVILEGES` omits the `SELECT` Postgres requires for `RETURNING`, accidentally correct today because every derived write table also has a read site; the derived side is **comment-blind** (`PF-219`'s own shape, closed on the declared side only); `resolveValues` returns `[]` with no `note()` when one hoisted constant is referenced twice at a call site; raw `$queryRaw` inside a scope stays a non-failing `[LIMIT]` (`PF-253`); and `apps/worker/src` is in the derivation's corpus but **not** in the widened trigger |
| **S-E01-1l** | The **SIXTH** module (`alerts`) enters the tenant scope — `meeting-requests` **whole**, `alerts.service.ts` on its **request path only**, and the first PARTIAL conversion of a single file said as such — and the privilege every write **LEADS WITH** (`RETURNING`, and the `WHERE` it reads) stops being accidental | 🟡 **shipped in part — 2026-08-23, ⚠️ NOT auto-merged (P1 · `[tenancy][security][authz][audit][rls][api][backend]`)** — [`stories/S-E01-1l.md`](./stories/S-E01-1l.md) | 70 | **Advances `PF-02`** (the *source* half), ships **`ADR-060`**, **closes `PF-254` + `PF-256` + `PF-258`**, records **`PF-257`** / **`PF-259`** / **`PF-260`** / **`PF-261`** / **`TOOL-41`**. **Read the title literally: this is a PARTIAL conversion, and `AC2` (« converts WHOLE ») is NOT met.** `meeting-requests.service.ts` converts **5/5** and reaches the `ADR-057 §D4` ratchet — its constructor holds only `TenantScopeService`, zero `this.prisma`. `alerts.service.ts` converts **26 of 32**; the other six are **enumerated per statement with their mechanism** (`evaluateAll`'s `alertRule.findMany`, `academicYear.findFirst`, `alertInstance.findFirst`, `alertInstance.create`, `notifyGuardiansOfAlert`'s `guardianship.findMany`, and `tenantsWithEnabledRules`' `alertRule.findMany`), so `AlertsService` **keeps** `PrismaService` and only ONE of the two classes reaches the constructor ratchet — named as a decision in `ADR-060 §D4` rather than faked by hiding the owner client behind a helper. The story sized this at *« ~106 scoped »*; **101 is the measured number**, and the shortfall is reported rather than rounded. Attribution re-derived, never edited: **`70 scoped + 120 enumerated / 818` → `101 + 127 / 818`** — **590** sites (was 628) would still return zero rows after the cutover. **`PF-254` closed and `PF-256` closed with it, deliberately as two rows:** `VERB_PRIVILEGES` is split by whether the statement **READS columns** — `RETURNING` (singular `create`/`update`/`delete`/`upsert` + `createManyAndReturn`) **and** `WHERE` (`update`/`updateMany`/`delete`/`deleteMany`, since Prisma compiles `updateMany` to `UPDATE … WHERE id IN (SELECT …)`); `createMany` is the only write verb that does neither, and closing `PF-254`'s row over its own residue would have re-created the defect one verb over. Measured cost repo-wide **167 → 170 pairs, `168 satisfied, 2 not`** — the two unsatisfied are the pre-existing `tenant/INSERT` + `tenant/UPDATE`: **zero new violations, free today and not free after one more module.** **Closure 38 → 47**, every one of the nine pairs read out of the checker's own `undeclared-pair` output and declared identically — never hand-guessed — then verified HELD on the running `pilotage_postgres` before declaration; **`47 declared === 47 derived`, both directions**. `audit_log` is the **first genuinely write-only** table to enter a scope (written at three sites, never read), which is exactly why the `RETURNING` gap had been *accidentally* correct: until now every written table also carried a read site contributing `SELECT`. It enters with **`SELECT` + `INSERT` only** — `UPDATE`/`DELETE` refused in the declaration and absent from the migration's `append_only` grant (`ADR-037 §D4` / GUARDRAILS §1: the hash chain must not become rewritable) — and the *POSITIONAL attribution refuses the phantom `audit_log.INSERT`* sentinel is **retired**, because its premise (« no `auditLog.create` in the corpus lives inside a scope ») is precisely what this slice falsifies. **G-AUDIT is UNCHANGED, and saying so is half the deliverable:** `alerts.service.ts` has **no** `$transaction`; all three audit writes were already post-mutation, best-effort, own-statement, swallow-and-continue, and each now gets its **own** scope with `try`/`catch` **outside** it. The naive single-scope reading would have made it strictly **worse** — a swallowed audit failure aborts the interactive transaction, `COMMIT` degenerates to `ROLLBACK`, and the acknowledge the row merely RECORDS is lost while the handler returns 200. **`PF-258` closed:** `resolveMeetingAssignee`'s method-wide `catch` now **rethrows** `TenantScopeError` / `ServiceUnavailableException` instead of degrading a config fault into a permanently persisted `assignedToId: null` — `POST /alerts/:id/meeting-intent` returns **503** where it previously returned 200 with an unassignable request. **Named limits, all recorded rather than glossed.** (1) The **third list is only half mechanised**: the append-only half is a DB-free ratchet (no declared pair may claim `UPDATE`/`DELETE` on an `APPEND_ONLY` table, non-vacuous now that `audit_log` is in the closure); membership in the 44 tenant tables was discharged **by measurement** on the container and stays open as **`PF-257`**, whose failure mode is unchanged and total — `refused_unusable` at boot ⇒ **503 on all four portals** while every existing check stays green. `S-E01-1k` called that half *« the must-do before the next module lands »*; this IS the next module, so the deferral has now happened twice — the boot probe is until then the last line of defence. (2) **`PF-260`**: `PRISMA_CALL_SITE_RE`'s negative lookbehind cannot see property-chain receivers, so **8 measured** `ctx.prisma.*` sites in `alerts/rules/*.rule.ts` are in neither the numerator nor the 818 denominator — the blind spot sits exactly where this slice declines to convert, so the `101 + 127 / 818` gauge reads better than it is. (3) The five `evaluateAll` statements are enumerated under `kind: 'bootstrap'`, a kind `ADR-051 §D2` defines as *identity and context resolution*; the escalation panel reads five of the seven reasons as arguing against a batch-wide scope **nobody proposed** (`FR6` prescribed one narrow scope per detection) and therefore as a scope reduction filed in `ENUMERATED_OUTSIDE_SCOPE` rather than left in the uncovered count — **recorded here, not re-litigated**, and it is the single thing a human should rule on first. (4) `alerts-scope-ownership.spec.ts`'s non-vacuity floor counts **22** `this.scope.run(` matches of which **2 are docblock prose**, so an editorial reword reddens it. (5) Both `list()` methods lose `Promise.all` — `findMany` (3-relation `include`) + `count` are now **sequential** inside one interactive transaction against the 5 s budget, correct for a single-connection tx, named in comments, **not measured**. (6) `loadMeetingRequestedAt` still swallows the classes `PF-258` judged unacceptable 300 lines earlier, so the rethrow rule is per-call-site and held only by prose. (7) Nothing here proves the running API **image** executes the new code. **No migration, no `schema.prisma` change, no `packages/contracts` change, no `apps/web` file, no `restore-drill-baseline.json` entry due (`PF-80` does not apply).** |
...
> **⚠️ POINTER MOVED 2026-08-23 by `S-E01-1l` (alerts → tenant scope). The block below is CONSUMED, not
> superseded, and one of its instructions was OVERRULED by measurement — say so plainly.**
>
> **The ledger disagreed, and here is the disagreement in full.** The `S-E01-1k` block below says the next slice is
> `S-E01-1l` — *agreed on identity* — *"BUT close the THIRD list first, in the same PR or the one immediately
> before it"*. This slice did **not** close the third list in full, and the reason is read from the migration rather
> than argued: `20260813120000_tenant_rls_policies` declares `append_only := ARRAY['audit_log',
> 'conversation_message']` and grants those two `SELECT, INSERT` while every other tenant table gets full DML.
> `alerts` derives on `audit_log` exactly `INSERT` and `SELECT` — **both granted** — and never derives
> `UPDATE`/`DELETE` on it, because the module is append-only by construction. So the ledger's hazard **cannot
> fire on this diff**, and the ordering instruction was a correct precaution aimed at a slice this one is not. The
> obligation was discharged **by measurement** instead (`information_schema.role_table_grants` on the running
> `pilotage_postgres`, output pasted in `ADR-060 §D3`), the half that needs no database **was** mechanised, and
> the remainder is recorded as **`PF-257`** rather than absorbed.
>
> **What landed here.** The **sixth** module enters the scope, and it is the **first partial** conversion of a
> single file — which is the honest shape, not a shortfall hidden in a number.
>
> ```
> before:  AC-9 CUTOVER READINESS: 70 scoped + 120 enumerated / 818
> after:   AC-9 CUTOVER READINESS: 101 scoped + 127 enumerated / 818
> ```
>
> `meeting-requests.service.ts` converts **5/5** and reaches the `ADR-057 §D4` ratchet (constructor holds only
> `TenantScopeService`). `alerts.service.ts` converts **26 of 32**; the other six are **enumerated per statement**
> with their mechanism: `evaluateAll` is an O(rules × detections) batch that fans out through seven
> `rules/*.rule.ts` evaluators, against a seam bounded at "≤ 2 statements" and a 5 s interactive transaction — it
> would raise **P2028** on the admin *"Lancer l'évaluation"* button and roll back the alerts already materialised —
> and `tenantsWithEnabledRules` takes **no** `tenantId` at all, because it PRODUCES the tenant set a scope keys
> on. A seventh enumerated entry is a declared **matcher artifact** (a docblock `tx.auditLog.create`, the PF-219
> shape). `AlertsService` therefore **keeps** `PrismaService`, named as a decision in `ADR-060 §D4` rather than
> faked by hiding the owner client behind a helper. The brief sized this at "~106 scoped"; **101 is the measured
> number** and the six statements that stayed out are named above.
>
> **`PF-254` closed, and `PF-256` closed with it — deliberately as two rows.** `VERB_PRIVILEGES` is split by
> whether the statement **READS columns**, which is not read/write symmetry and is written as the reason in the
> source: `RETURNING` (singular `create`/`update`/`delete`/`upsert` + `createManyAndReturn`) **and**
> `WHERE` (`update`/`updateMany`/`delete`/`deleteMany` — Prisma compiles `updateMany` to
> `UPDATE … WHERE id IN (SELECT …)`). `createMany` is the only write verb that does neither. The ledger's
> instruction (2) covered only the `RETURNING` half; taking it alone would have closed `PF-254`'s row over its own
> residue, so the `WHERE` half is filed as `PF-256` and fixed in the same diff. **Measured cost: repo-wide
> 167 → 170 (table, privilege) pairs, `168 satisfied, 2 not`** — the two unsatisfied are the pre-existing
> `tenant/INSERT` + `tenant/UPDATE`. **Zero new violations: free today, not free after one more module.**
>
> **Why alerts and not a later module made it load-bearing.** `audit_log` is the **first genuinely write-only**
> table to enter a scope — this module writes it at three sites and never reads it. Until now every written table in
> the derived set also carried a read site contributing `SELECT`, so the mapping was **accidentally** correct.
> **Latent, not live, and not inflated:** `app_user` holds `SELECT, INSERT` on `audit_log` deliberately
> (ADR-037 §D4 / GUARDRAILS §1 — the hash chain must not become rewritable), so the runtime works and no incident was
> possible on a correctly-migrated cluster. What was wrong was the **derivation**, which would have declared
> `INSERT` alone, certified `enforcing: true`, and left any cluster hardened to `GRANT INSERT` booting green
> into a 42501 on its first audit write.
>
> **Closure 38 → 47**, all nine pairs **read out of the checker's own `undeclared-pair` output** and declared
> identically — never hand-guessed — then verified HELD on the running container before declaration. Bidirectional
> set equality holds: `47 declared === 47 derived`. The `POSITIONAL attribution refuses the phantom
> audit_log.INSERT` sentinel is **retired**: it existed because no `auditLog.create` in the corpus lived inside a
> scope, and a sentinel that outlives its premise stops testing the mechanism and starts failing correct work.
>
> **G-AUDIT, stated plainly so it cannot be read as an upgrade.** `alerts.service.ts` has **no** `$transaction`;
> all three audit writes were already post-mutation, best-effort, own-statement, swallow-and-continue. Each now gets
> its **own** scope with the `try`/`catch` outside it. The transactional relationship is **unchanged — it was
> never atomic.** The naive single-scope reading would have made it strictly worse: a swallowed audit failure aborts
> the interactive transaction, `COMMIT` degenerates to `ROLLBACK`, and the acknowledge/resolve the row merely
> RECORDS is lost while the handler returns 200.
>
> **→ The next slice: close `PF-257` (the remaining half of the third list) and then take the seventh module.**
> The append-only half is ratcheted here without a database; what remains is asserting membership in the 44
> tenant-scoped tables, parsing the migration's SQL arrays the way `ADR-059 §D1` parses `tenant-scope.ts`. Its
> failure mode is unchanged and total: a declared pair the matrix withholds returns `refused_unusable` and 503s
> **all four portals** while every existing check stays green.
>
> **Behind it, in the order to take them.** (1) **`PF-260`** — `PRISMA_CALL_SITE_RE` cannot see `ctx.prisma`:
> **measured 8** production sites in `alerts/rules/*.rule.ts` hitting `grade`, `attendance_record`, `student`,
> none of them in the 818 and none checked against `role_table_grants`. Take it **before** converting
> `evaluateAll`, or that conversion can leave 8 owner statements inside an open scope with no
> `owner-inside-scope` report. (2) **`TOOL-41`** — hoist the `ADR-058 §D1` lexical ratchet out of per-module
> specs; `alerts` alone adds six swallow-and-continue sites against `remediation`'s three, and `student-portal`
> still has no ownership spec at all. (3) **`PF-261`** — `ensureRules` can duplicate tenant-level rules
> (`@@unique` with a nullable `schoolId` does not fire) and a `GET` performs an `INSERT`; needs a migration,
> therefore its own slice **and** a `restore-drill-baseline.json` entry (PF-80). (4) **`PF-249` / `PF-250` /
> `PF-252` / `PF-253`**, unchanged. `S-E01-1` — the global `DATABASE_URL` flip — still comes after all of it,
> on **590** unconverted sites (was 628).

---

> **⚠️ POINTER MOVED 2026-08-22 by `S-E01-1k` (run 69), and the previous block's pointer is CONSUMED, not superseded.**
> The `S-E01-1j` block below named *"`PF-246` / `PF-219` — DERIVE the privilege closure instead of writing it, and
> take it NOW"*. That is exactly what landed here. It is kept below as dated history.
>
> **⚠️ ID RE-ASSIGNMENT, read this before selecting.** The block below allocates the id **`S-E01-1k`** to *"the next
> module … `alerts.service.ts` is the best-shaped candidate"*. **That allocation is withdrawn.** `S-E01-1k` is the
> derivation slice that shipped in run 69; the `alerts.service.ts` conversion is re-assigned to **`S-E01-1l`** in this
> file and in `bmad/roadmap.md`. Do **not** re-select `S-E01-1k` — it is claimed. (This is the `PF-185`/`PF-186`/
> `TOOL-27` collision pattern caught before it reached a second implementation, not after.)
>
> **What landed here.** `APP_ROLE_REQUIRED_PRIVILEGES` stops being a list that only re-reads itself. A derivation
> over the call sites attributed **exactly `scoped`**, descending relation depth through `where` / `select` /
> `include` / `orderBy` / `_count.select` and resolving each hop against `schema.prisma`, is compared to the declared
> list for **set equality in both directions** and wired to `fail()`. It found `guardian.SELECT` on its first pass —
> closure **37 → 38** — a relation-filter read in the parent ABAC guard of `lessons/list` that is a root delegate
> **nowhere** in the tree, which is why five slices and three human re-reads never saw it. Ships **`ADR-059`**;
> `ci-gate.sh`'s tenant-adversarial trigger widens to `apps/api/src/modules/` and `scripts/lib/` (`§D3`).
>
> **→ The next slice is `S-E01-1l` — convert `alerts.service.ts` (33 call sites), BUT close the THIRD list first, in
> the same PR or the one immediately before it.** The reason is measured and it is this slice's own named gap.
> `S-E01-1k` mechanised **code ⟷ declared**. It left **declared ⟷ provisioned** hand-written, and that is now the only
...
> instructs the reader to declare it, `appRoleVerdict` then returns `refused_unusable`, and **every existing check
> stays green**. Two shapes are on the table and either closes it: (a) assert `declared ⊆ granted-matrix` in
> `tenant-adversarial-check.js` right after the `AC-2` matrix block (~10 lines, `expectedMatrix` already in scope), or
> (b) the DB-free unit version in `tenant-scope.spec.ts`, parsing the committed migrations' SQL arrays with the
> `sqlArrayNames` reader `rls-isolation-gate.spec.ts:311` already proves works. **Verified green on all 38 entries
> today**, so it lands as a ratchet, not as a repair.
>
> **Behind it, in the order to take them.** (1) **`PF-252`** — thread `parseDerivedChildParents(...).problems` into
> the verdict; it is computed and **discarded** at `tenant-adversarial-check.js:3784`, so renaming one hard-coded
> migration directory silently disarms the parent-SELECT rule while the check prints a green *"0 derived-child
> policies read"*. (2) **The `RETURNING` gap in `VERB_PRIVILEGES`** — `create`/`update`/`delete`/`upsert` emit
> `RETURNING`, which Postgres requires `SELECT` for; accidentally correct today only because every derived write table
> also has a read site, and now load-bearing for a bidirectional set equality wired to `fail()`. (3) **`PF-219`'s
> remaining half** — the *derived* side is comment-blind: `PRISMA_CALL_SITE_RE` runs over raw text, so a `tx.x.y(` in
> a docblock inside a scope contributes a real pair. (4) **`PF-250`** — split the pure source-to-source closure
> comparison out of the 300 s DB-bearing stage into its own sub-second `run_stage` carrying the wide trigger, so
> `§D3`'s benefit costs ~0 s and "Postgres is down" stops sharing a colour with "tenancy is broken". (5) **`PF-248`**
> (P1, narrow the three fail-soft catches), **`PF-247`** (make `ADR-058 §D1` observable), **`PF-253`** (raw SQL inside
> a scope), **`PF-224`** (P1, `azp`/`aud` at the API), **`PF-243`** (P2, arm `R-05` locally). `S-E01-1` — the global
> `DATABASE_URL` flip — still comes after all of it, on **628** unconverted sites.
