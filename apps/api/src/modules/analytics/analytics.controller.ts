...
export class AnalyticsController {
  ...
  async snapshotRebuild(
    ...
    // S-E04-1 — one derivation, one home. The inline role-precedence copy that
    // used to sit here (and the literal `portal: 'admin'` it fed at
    // `snapshot-ops.service.ts`) are gone: `portal` is now derived from the same
    // JWT read and threaded down, so a teacher-role rebuild is no longer audited
    // as an admin-portal action.
    const { actorRole, portal } = deriveAuditProvenance(jwt);
    return this.snapshotOps.enqueueRebuild({
      tenantId,
      actorId: me.id,
      actorRole,
      portal,
