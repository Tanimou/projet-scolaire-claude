...
/**
 * NEGATIVE_TREND — fires when a student's running average in a subject drops
 * by at least `delta` points /20 across the last `windowAssessments`
 * consecutive published evaluations. The trailing window is split in two
 * halves (earlier vs later) in memory — over the SAME single `grade.findMany`
 * the sibling rules run, no extra query — and the rule fires only on a
 * downward move: `firstHalfAvg - lastHalfAvg >= delta`.
 * Defaults: baisse de >= 1.5 pts sur 3 évaluations consécutives.
 */
export async function evaluateNegativeTrend(ctx: RuleContext): Promise<DetectedAlert[]> {
  ...
  // on a real downward trend" guarantee for ANY value. `delta` must stay > 0 —
  // a 0/negative delta would fire on a flat or rising series. The window needs
  ...
  for (const [key, arr] of groups) {
    ...
    const drop = firstHalfAvg - lastHalfAvg;
    // Fire ONLY on a downward trend (drop >= delta). Flat / rising series produce nothing.
    if (drop < delta) continue;
    ...
    out.push({
      ...
      title: `Tendance négative en ${first.subjectName}`,
      body: `Baisse de ${drop.toFixed(1)} pts /20 sur les ${windowAssessments} dernières évaluations en ${first.subjectName} (de ${firstHalfAvg.toFixed(1)} à ${lastHalfAvg.toFixed(1)} /20).`,
      recommendation:
        "Un échange avec l'enseignant·e de la matière peut aider à enrayer la baisse avant qu'elle ne s'installe.",
      context: {
