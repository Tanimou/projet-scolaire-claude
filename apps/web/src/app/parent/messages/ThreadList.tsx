...
/**
 * ThreadList — the E2-S2 parent inbox rows. A pure, server-renderable
 * presentational component (no `'use client'`): each row is a `Link` to
 * `/parent/messages/[id]`, so the whole inbox renders from ONE aggregate
 * `GET /api/v1/conversations` call (no client N+1).
 *
 * Per the Sally UX spec: 64px-min rows, violet messaging accent, an unread
 * treatment conveyed by THREE non-colour cues (left accent bar + bold name +
 ...
 * combining teacher + child + unread count so each row is intelligible out of
 * visual context.
 */

/** Teacher initials for the leading avatar chip (max 2 glyphs). */
...
function rowAriaLabel(c: ConversationDto): string {
  const base = `Conversation avec ${c.teacherName} au sujet de ${c.studentName}`;
  if (c.unreadCount > 0) {
...
export function ThreadList({ conversations }: { conversations: ConversationDto[] }) {
  return (
      ...
      {conversations.map((c) => {
        ...
        return (
          <li key={c.id}>
            ...
            >
              {/* Leading avatar chip — teacher initials, violet messaging accent. */}
              <span
                aria-hidden
              ...
              <span className="min-w-0 flex-1">
                {/* Line 1 — teacher name + relative timestamp. */}
                ...
                <span className="mt-0.5 flex flex-wrap items-center gap-1.5">
                  <span className="text-xs text-slate-500">{c.studentName}</span>
                  {c.alertContext && (
