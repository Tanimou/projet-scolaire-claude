 ...
 *
 * ─── S-E03-8 / PF-40 ────────────────────────────────────────────────────────
 * Ce panneau portait la CINQUIÈME déclaration de « à venir » du dépôt
 * (`endsAt >= Date.now()`, tri, coupe) et une TROISIÈME définition de
 * « semaine » (`WEEK_MS` glissant), voisine de la semaine ISO de la page
 * calendrier du même portail : un événement pouvait être peint « imminent » ici
 * et absent du compteur « CETTE SEMAINE » là-bas. Les deux prédicats viennent
 * maintenant de `@pilotage/contracts`.
 *
 * Ce composant s'exécute CÔTÉ SERVEUR (pas de `'use client'`), donc son
 * `Date.now()` ne divergeait pas entre SSR et hydratation — mais il lisait une
 * horloge DIFFÉRENTE de celle du reste de la page. L'ancre arrive donc en prop,
 * comme partout ailleurs : un seul instant par requête.
 *
 * Le titre promet « prochains événements » et la carte n'en montre que six : le
 * lien d'en-tête porte désormais le VRAI total, ce qui transforme un plafond
 * silencieux (`DNC-06`) en invitation.
 */
