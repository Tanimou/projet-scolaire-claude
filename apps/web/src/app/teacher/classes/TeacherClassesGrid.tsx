...
/** Neutral slate fallback when a cycle has no configured colour. */
const FALLBACK_CYCLE_COLOR = '#cbd5e1';
...
// =============================================================================
// Pure filter / sort / option helpers — kept out of the component so the data
// flow reads as a pipeline and the logic stays unit-testable in isolation.
// =============================================================================

function buildCycleOptions(classes: ClassCardData[]): SelectOption[] {
...
function buildSubjectOptions(classes: ClassCardData[]): SelectOption[] {
  const seen = new Map<string, string>();
...
function filterClasses(
  ...
  { q, cycle, subject }: { q: string; cycle: string; subject: string },
): ClassCardData[] {
...
function sortClasses(classes: ClassCardData[], sort: SortKey): ClassCardData[] {
  const sorted = [...classes];
  switch (sort) {
    case 'students-desc':
      sorted.sort(
        (a, b) => b.enrolledCount - a.enrolledCount || a.className.localeCompare(b.className),
      );
      break;
    case 'fill-desc':
...
// =============================================================================
// Presentational card — one teaching class with capacity, subjects & actions.
// =============================================================================

function ClassCard({ c }: { c: ClassCardData }) {
  const cycleColor = c.cycleColor ?? FALLBACK_CYCLE_COLOR;

  return (
    <li className="relative overflow-hidden rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60 transition hover:-translate-y-0.5 hover:ring-slate-300">
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {c.subjects.map((s) => (
          <Link
            ...
            title={`Ouvrir le carnet de notes ${s.name}`}
...
// =============================================================================
// Grid + filter bar — owns the filter state and renders the result pipeline.
// =============================================================================
...
export function TeacherClassesGrid({ classes }: { classes: ClassCardData[] }) {
  ...
  // stale options.
  const cycleOptions = useMemo<SelectOption[]>(() => buildCycleOptions(classes), [classes]);
  const subjectOptions = useMemo<SelectOption[]>(() => buildSubjectOptions(classes), [classes]);

  const visible = useMemo(
    () => sortClasses(filterClasses(classes, { q, cycle, subject }), sort),
      ...
      <div className="mt-4">
        {visible.length === 0 ? (
          <EmptyState
            ...
            tone="slate"
            action={
              hasActiveFilters ? { label: 'Réinitialiser les filtres', onClick: reset } : undefined
            }
          />
        ) : (
          <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {visible.map((c) => (
              <ClassCard key={c.classSectionId} c={c} />
            ))}
          </ul>
        )}
      </div>
