...
export async function createTeacherAnnouncement(
  ...
  try {
    const data = await api<{ id: string }>('/api/v1/announcements', {
      method: 'POST',
      body: payload,
    });
...
export async function publishTeacherAnnouncement(id: string): Promise<Result> {
  try {
...
export async function deleteTeacherAnnouncement(id: string): Promise<Result> {
  try {
